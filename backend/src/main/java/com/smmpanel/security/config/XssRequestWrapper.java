 ...
 *
 * <p>This wrapper intercepts parameter values, headers, and request body content to remove or
 * neutralize common XSS attack patterns.
 *
 * <p>Features: - Parameter value filtering - Request body filtering - Header value filtering -
 * Configurable XSS patterns - Logging of filtered content for security monitoring
 */
@Slf4j
public class XssRequestWrapper extends HttpServletRequestWrapper {
    ...
    private String filterXss(String input) {
        ...
        if (wasFiltered || !input.equals(filteredInput)) {
            log.warn(
                    "XSS content filtered from request. Original length: {}, Filtered length: {},"
                            + " Remote address: {}",
                    input.length(),
                    filteredInput.length(),
                    getRemoteAddr());
            log.debug(
                    "XSS filtered content (first 100 chars): {}",
