 ...
 * 
 * IMPROVEMENTS:
 * 1. Uses RateLimitService for proper rate limiting
...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) 
        ...
        try {
            ...
            response.getWriter().write("{\"error\":\"Rate limit exceeded\",\"message\":\"" + e.getMessage() + "\"}");
        }
    }
    ...
    private void addRateLimitHeaders(HttpServletResponse response, String userId, String operation) {
        try {
            RateLimitService.RateLimitStatus status = rateLimitService.getRateLimitStatus(userId, operation);
