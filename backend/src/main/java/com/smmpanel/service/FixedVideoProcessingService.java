...
public class FixedVideoProcessingService {
    ...
    public void completeVideoProcessing(Long orderId, String finalUrl, boolean clipCreated) {
        try {
                            ...
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    "Video processing not found for order: "
                                                            + orderId));
            ...
            log.info(
                    "Completed video processing for order {} with final URL: {}",
                    orderId,
                    finalUrl);

        } catch (Exception e) {
            log.error(
                    "Failed to complete video processing for order {}: {}",
                    orderId,
                    e.getMessage(),
                    e);
            throw e;
    ...
    private void assignOfferAutomatically(Long orderId, String targetUrl, boolean clipCreated) {
        try {
            ...
            log.info(
                    "Initiated automatic offer assignment for order {} (clip: {})",
                    orderId,
                    clipCreated);

        } catch (Exception e) {
            log.error(
                    "Failed to initiate automatic offer assignment for order {}: {}",
                    orderId,
                    e.getMessage(),
                    e);
    ...
    public void handleVideoProcessingError(Long orderId, String errorMessage) {
        try {
            ...
            log.error(
                    "Failed to handle video processing error for order {}: {}",
                    orderId,
                    e.getMessage(),
                    e);
        }
