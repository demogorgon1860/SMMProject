...
/**
 * User Balance Cache Service Implements write-through caching strategy for user balances 5-minute
 * TTL as per optimization requirements
 */
@Slf4j
@Service
...
public class UserBalanceCacheService {
    ...
    /**
     * Get cached user balance with 5-minute TTL Cache miss will return null and should be populated
     * by database lookup
     */
    @Cacheable(
            value = "user-balances",
    ...
    /**
     * Update user balance in cache (write-through) This method should be called whenever balance is
     * updated in database
     */
    @CachePut(
            value = "user-balances",
    ...
    /**
     * Evict user balance from cache Should be called when balance operation fails or for cache
     * invalidation
     */
    @CacheEvict(
            value = "user-balances",
    ...
    /** Preload user balance into cache Used for warming cache after database operations */
