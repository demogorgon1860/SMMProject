...
/**
 * Test class to verify that controller layer optimizations prevent N+1 queries and use proper batch
 * fetching strategies
 */
@SpringBootTest
@ActiveProfiles("test")
...
public class ControllerQueryOptimizationTest {
    ...
    public void testOptimizedOrderListingPreventsNPlusOne() {
        ...
        Page<Order> orders =
                orderRepository.findOrdersWithDetailsByUserId(
                        users.get(0).getId(), PageRequest.of(0, 10));
    ...
    public void testBatchFetchingForMultipleOrders() {
        ...
        List<Long> orderIds = orders.stream().limit(5).map(Order::getId).toList();
    ...
    public void testQueryCountForUserTransactions() {
        ...
        assertTrue(
                fetchQueries <= 1,
                "Should use at most 1 query to fetch transactions with relations");
    }
