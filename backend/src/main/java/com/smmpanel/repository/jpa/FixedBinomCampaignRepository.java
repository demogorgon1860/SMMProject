...
/**
 * CRITICAL: Repository for Fixed Binom Campaigns MUST support exactly 3 campaign distribution for
...
public interface FixedBinomCampaignRepository extends JpaRepository<FixedBinomCampaign, Long> {
    ...
    @Query(
            "SELECT f FROM FixedBinomCampaign f WHERE f.active = true "
    ...
    @Query(
            "SELECT f FROM FixedBinomCampaign f WHERE f.active = true ORDER BY f.priority ASC,"
                    + " f.weight DESC")
