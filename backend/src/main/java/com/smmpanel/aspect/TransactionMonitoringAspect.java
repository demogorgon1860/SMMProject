...
@Aspect
@Component
@ConditionalOnProperty(name = "app.transaction.monitoring.enabled", havingValue = "true", matchIfMissing = false)
public class TransactionMonitoringAspect {

    private final AtomicLong transactionCounter = new AtomicLong(0);
    private final ConcurrentHashMap<String, TransactionMetrics> transactionMetrics = new ConcurrentHashMap<>();
    ...
    private Object monitorTransaction(ProceedingJoinPoint joinPoint, String category) throws Throwable {
        ...
        log.debug("Transaction {} started - Method: {}, Category: {}, Active: {}, Isolation: {}, ReadOnly: {}", 
            transactionId, methodName, category, wasTransactionActive, isolationLevel, isReadOnly);

        TransactionMetrics metrics = transactionMetrics.computeIfAbsent(methodName, k -> new TransactionMetrics());
        ...
        try {
            ...
            log.debug("Transaction {} completed successfully - Method: {}, Duration: {}ms, Still Active: {}", 
                transactionId, methodName, duration, isStillActive);

            // Log long-running transactions
            if (duration > 5000) {
                log.warn("Long-running transaction detected - Method: {}, Duration: {}ms, Category: {}", 
                    methodName, duration, category);
            }
    ...
    private String getIsolationLevel() {
        try {
            if (TransactionSynchronizationManager.isActualTransactionActive()) {
                // This is a simplified approach - in practice, you might need to access the transaction definition
                return "ACTIVE";
            }
            return "NONE";
        } catch (Exception e) {
    ...
    public static class TransactionMetrics {
        ...
        public long getMinDuration() { return minDuration.get() == Long.MAX_VALUE ? 0 : minDuration.get(); }
        ...
        public String toString() {
            return String.format("TransactionMetrics{attempts=%d, successes=%d, failures=%d, avgDuration=%.2fms, maxDuration=%dms, successRate=%.2f%%}",
                getAttempts(), getSuccesses(), getFailures(), getAverageDuration(), getMaxDuration(), getSuccessRate());
        }
    }
}