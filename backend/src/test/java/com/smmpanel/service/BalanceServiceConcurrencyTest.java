...
class BalanceServiceConcurrencyTest {
    ...
    void testConcurrentBalanceDeductions() throws InterruptedException {
        ...
        BigDecimal expectedRemainingBalance =
                new BigDecimal("1000.00")
                        .subtract(
                                deductionAmount.multiply(
                                        new BigDecimal(successfulDeductions.get())));
        ...
        assertEquals(
                numberOfThreads * deductionsPerThread,
                        ...
                        try {
                            for (int j = 0; j < additionsPerThread; j++) {
                                balanceService.addBalance(
                                        testUser, additionAmount, null, "Concurrent test addition");
    ...
    void testConcurrentBalanceTransfers() throws InterruptedException {
        ...
        assertTrue(
                latch.await(30, TimeUnit.SECONDS),
                "Transfer operations should complete within 30 seconds");
        executor.shutdown();
    ...
    void testAtomicBalanceAdjustment() throws InterruptedException {
                    ...
                    () -> {
                        try {
                            BigDecimal adjustment =
                                    adjustments.get(threadIndex % adjustments.size());
        ...
        assertTrue(
                latch.await(30, TimeUnit.SECONDS),
                "Adjustment operations should complete within 30 seconds");
        executor.shutdown();
    ...
    void testCheckAndReserveBalanceConcurrency() throws InterruptedException {
        ...
        assertTrue(
                latch.await(30, TimeUnit.SECONDS),
                "Balance check operations should complete within 30 seconds");
        executor.shutdown();
    ...
    void testRetryMechanismWithSimulatedOptimisticLockingFailure() {
        ...
        assertDoesNotThrow(
                () -> {
                    balanceService.deductBalance(
                            testUser, new BigDecimal("50.00"), null, "Test deduction");
                });
    ...
    void testBalanceConsistencyAfterHighConcurrency() throws InterruptedException {
                            ...
                            for (int j = 0; j < operationsPerThread; j++) {
                                try {
                                    switch ((threadIndex + j) % 4) {
                                        ...
                                        case 3:
                                            balanceService.checkAndReserveBalance(
                                                    testUser.getId(), new BigDecimal("10.00"));
                                            break;
                                    }
                                } catch (Exception e) {
                                    // Log but don't fail the test for expected exceptions
                                    System.out.println(
                                            "Expected exception during stress test: "
                                                    + e.getMessage());
                                }
                            }
                        } finally {
        ...
        assertTrue(
                latch.await(60, TimeUnit.SECONDS), "Stress test should complete within 60 seconds");
        executor.shutdown();
        ...
        assertTrue(
                finalUser.getBalance().compareTo(BigDecimal.ZERO) >= 0,
