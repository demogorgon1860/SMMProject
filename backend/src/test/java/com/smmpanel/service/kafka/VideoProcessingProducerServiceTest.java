 ...
 *
 * <p>Tests the Kafka producer service functionality: 1. Message sending with proper formatting 2.
 * Error handling and retries 3. Metrics tracking 4. Batch processing
 */
@ExtendWith(MockitoExtension.class)
class VideoProcessingProducerServiceTest {

    @Mock private KafkaTemplate<String, VideoProcessingMessage> kafkaTemplate;

    @Mock private SendResult<String, VideoProcessingMessage> sendResult;
    ...
    void testSendStandardProcessingMessage() {
        ...
        verify(kafkaTemplate, times(1))
                .send(
                        argThat(
                                (Message<VideoProcessingMessage> message) -> {
                                    VideoProcessingMessage payload =
                                            (VideoProcessingMessage) message.getPayload();
                                    return payload.getOrderId().equals(TEST_ORDER_ID)
                                            && payload.getVideoId().equals(TEST_VIDEO_ID)
                                            && payload.getPriority()
                                                    == VideoProcessingMessage.ProcessingPriority
                                                            .MEDIUM;
                                }));
    }
                                ...
                                (Message<VideoProcessingMessage> message) -> {
                                    ...
                                    return payload.getOrderId().equals(TEST_ORDER_ID)
                                            ...
                                            && payload.getMaxAttempts()
                                                    == 5; // High priority gets more retries
                                }));
    }
                        ...
                        argThat(
                                (Message<VideoProcessingMessage> message) -> {
                                    ...
                                    return payload.getOrderId().equals(TEST_ORDER_ID)
                                            ...
                                            && "true"
                                                    .equals(payload.getMetadata().get("scheduled"));
                                }));
    }
    ...
    void testSendBatchMessages() {
        // Arrange
        List<VideoProcessingMessage> messages =
                List.of(
                        VideoProcessingMessage.createStandardMessage(
                                1L, "video1", "url1", 100, 10L),
                        VideoProcessingMessage.createStandardMessage(
                                2L, "video2", "url2", 200, 20L),
                        VideoProcessingMessage.createStandardMessage(
                                3L, "video3", "url3", 300, 30L));
                ...
                .send(
                        argThat(
                                (Message<VideoProcessingMessage> message) -> {
                                    VideoProcessingMessage payload =
                                            (VideoProcessingMessage) message.getPayload();
                                    return payload.getProcessingConfig()
                                                    .containsKey("custom-setting")
                                            && payload.getProcessingConfig()
                                                    .get("numeric-setting")
                                                    .equals(42)
                                            && payload.getProcessingConfig()
                                                    .get("boolean-setting")
    ...
    void testHealthCheckFailure() {
        // Arrange
        when(kafkaTemplate.partitionsFor(TEST_TOPIC))
