package com.smmpanel.service.user;

import com.smmpanel.entity.User;
import com.smmpanel.repository.jpa.UserRepository;
import com.smmpanel.service.core.OptimisticLockingService;
...
public class UserService {
                        ...
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "Invalid API key or account disabled"));
    ...
    public User getUserByApiKey(String apiKey) {
        return userRepository
                .findByApiKeyHashAndIsActiveTrue(apiKey)
                .orElseThrow(
                        () -> new IllegalArgumentException("Invalid API key or account disabled"));
    }
    ...
    public User updateBalanceWithLocking(Long userId, BigDecimal amount, String description) {
        return optimisticLockingService.handleBalanceUpdate(
                userId,
                () -> {
                    ...
                    log.debug(
                            "Updating balance for user {} (version {}): {} -> {}",
                            userId,
                            user.getVersion(),
                            user.getBalance(),
                    ...
                    log.info(
                            "Balance updated for user {} (version {} -> {}): {} ({})",
                            userId,
                            user.getVersion(),
     ...
     */
    @Transactional
    public User updateUserWithLocking(
            Long userId, java.util.function.Function<User, User> updates) {
    ...
    public User deductBalanceForOrder(Long userId, BigDecimal orderAmount, Long orderId) {
        return updateBalanceWithLocking(
                userId, orderAmount.negate(), "Order payment - Order ID: " + orderId);
    }
    ...
    public User refundBalanceForOrder(Long userId, BigDecimal refundAmount, Long orderId) {
        return updateBalanceWithLocking(
                userId, refundAmount, "Order refund - Order ID: " + orderId);
    }
    ...
    public User addDeposit(Long userId, BigDecimal depositAmount, String transactionId) {
        return updateBalanceWithLocking(
                userId, depositAmount, "Deposit - Transaction ID: " + transactionId);
    }
