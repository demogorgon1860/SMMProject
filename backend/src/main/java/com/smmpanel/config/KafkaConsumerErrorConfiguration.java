 ...
 *
 * <p>Provides comprehensive error handling for Kafka consumers: 1. Configurable retry policies with
 * exponential/fixed backoff 2. Dead Letter Queue (DLQ) publishing with metadata enhancement 3.
 * Exception classification and handling strategies 4. Error metrics and monitoring 5. Recovery
 * mechanisms for different failure scenarios
 */
@Slf4j
@Configuration
...
public class KafkaConsumerErrorConfiguration {
    ...
    public CommonErrorHandler defaultKafkaErrorHandler() {
        log.info(
                "Configuring Kafka error handler with {} retries, {}ms initial interval, {}ms max"
                        + " interval",
                maxRetries,
                initialInterval,
                maxInterval);
        ...
        DeadLetterPublishingRecoverer recoverer =
                new DeadLetterPublishingRecoverer(kafkaTemplate, this::resolveDlqDestination);
        ...
        recoverer.setHeadersFunction(
                (consumerRecord, exception) -> {
                    ...
                    headers.add(
                            "dlt-exception-class", exception.getClass().getSimpleName().getBytes());
        ...
        errorHandler.setRetryListeners(
                (consumerRecord, exception, deliveryAttempt) -> {
                    ...
                    log.warn(
                            "Retrying message processing (attempt {}/{}): topic={}, partition={},"
                                    + " offset={}, error={}",
                            deliveryAttempt,
                            maxRetries,
        ...
        DefaultErrorHandler errorHandler =
                new DefaultErrorHandler(
                        (consumerRecord, exception) -> {
                            ...
                            log.error(
                                    "DLQ message processing failed permanently: topic={},"
    ...
    public CommonErrorHandler orderProcessingErrorHandler() {
        ConsumerRecordRecoverer orderRecoverer =
                (consumerRecord, exception) -> {
                    dlqSentCount.incrementAndGet();
                ...
                (consumerRecord, exception, deliveryAttempt) -> {
                    ...
                    if (deliveryAttempt == 1) {
                        ...
                        if (isNonRetryableException(exception)) {
                            ...
                            log.warn(
                                    "Non-retryable exception encountered: topic={}, partition={},"
                                            + " offset={}, error={}",
                                    consumerRecord.topic(),
                                    consumerRecord.partition(),
                                    consumerRecord.offset(),
    ...
    private TopicPartition resolveDlqDestination(
    ...
    private void handleOrderProcessingFailure(
            ConsumerRecord<?, ?> consumerRecord, Exception exception) {
        try {
            // Extract order information if possible
            log.error(
                    "Handling order processing failure for record: topic={}, partition={},"
                            + " offset={}",
                    consumerRecord.topic(),
                    consumerRecord.partition(),
                    consumerRecord.offset());
    ...
    private void sendDlqProcessingFailureAlert(
            ConsumerRecord<?, ?> consumerRecord, Exception exception) {
        log.error(
                "CRITICAL: DLQ processing failed - manual intervention required: topic={},"
                        + " partition={}, offset={}",
                consumerRecord.topic(),
                consumerRecord.partition(),
                consumerRecord.offset());
    ...
    public Map<String, Object> kafkaErrorMetrics() {
        return Map.of(
                ...
                "errorRate",
                        totalErrorsCount.get() > 0
                                ? (double) nonRetryableErrorsCount.get() / totalErrorsCount.get()
                                : 0.0);
    }
}
