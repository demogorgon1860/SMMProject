...
class ConcurrentOrderCreationTest {
    ...
    void setUp() {
        ...
        testUser = createUserWithApiKey("ordertest", "order@test.com", new BigDecimal("1000.00"), "test-api-key");
        testApiKey = "test-api-key";

        // Create multiple users for complex scenarios
        multipleUsers = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            multipleUsers.add(createUserWithApiKey("orderuser" + i, "orderuser" + i + "@test.com", 
                new BigDecimal("500.00"), "api-key-" + i));
        }
    ...
    void testConcurrentOrderCreationSameUser() throws InterruptedException {
        ...
        assertTrue(latch.await(120, TimeUnit.SECONDS), "Concurrent order creation should complete within 2 minutes");
        executor.shutdown();
        ...
        assertEquals(successfulOrders.get(), transactions.size(), 
        ...
        log.info("Same user test completed - Successful: {}, Failed: {}, Insufficient balance: {}, " +
                "Final balance: {}, Total exceptions: {}", 
                successfulOrders.get(), failedOrders.get(), insufficientBalanceErrors.get(), 
                updatedUser.getBalance(), exceptions.size());

        // Ensure reasonable success rate
        double successRate = (double) successfulOrders.get() / (successfulOrders.get() + failedOrders.get()) * 100;
    ...
    void testConcurrentOrderCreationDifferentUsers() throws InterruptedException {
                ...
                try {
                    for (int j = 0; j < ordersPerUser; j++) {
                        try {
                            ...
                            log.debug("Order creation failed for user {}: {}", user.getUsername(), e.getMessage());
                        }
                    }
                } finally {
                    latch.countDown();
        ...
        assertTrue(latch.await(90, TimeUnit.SECONDS), "Multi-user order creation should complete within 90 seconds");
        executor.shutdown();
        ...
        log.info("Multi-user test completed - Total successful: {}, Total failed: {}", 
                totalSuccessfulOrders.get(), totalFailedOrders.get());
    }
    ...
    void testConcurrentOrderCreationWithInsufficientBalance() throws InterruptedException {
        ...
        User limitedUser = createUserWithApiKey("limited", "limited@test.com", 
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            executor.submit(() -> {
                ...
                try {
                    ...
                    log.debug("Unexpected error in thread {}: {}", threadIndex, e.getMessage());
                } finally {
        ...
        assertTrue(latch.await(150, TimeUnit.SECONDS), "Insufficient balance test should complete within 150 seconds");
        executor.shutdown();

        // Verify results
        User updatedUser = userRepository.findById(limitedUser.getId()).orElseThrow();
        
        // Should only allow orders that fit within balance (200/500 = 0, but might allow partial if any balance remains)
        assertTrue(successfulOrders.get() <= 1, "Should allow at most 1 expensive order with limited balance");
        assertTrue(insufficientBalanceCount.get() >= numberOfThreads - 1, 
        ...
        log.info("Insufficient balance test completed - Successful: {}, Insufficient balance: {}, " +
                "Other errors: {}, Final balance: {}, Average execution time: {}ms", 
                successfulOrders.get(), insufficientBalanceCount.get(), otherErrors.get(), 
                updatedUser.getBalance(), totalExecutionTime.get() / numberOfThreads);
    }
    ...
    void testConcurrentOrderCreationStressTest() throws InterruptedException {
                ...
                try {
                    for (int j = 0; j < ordersPerThread; j++) {
                        ...
                        try {
                            ...
                            User randomUser = multipleUsers.get(ThreadLocalRandom.current().nextInt(multipleUsers.size()));
                            String apiKey = "api-key-" + multipleUsers.indexOf(randomUser);
                            
                            com.smmpanel.entity.Service randomService = ThreadLocalRandom.current().nextBoolean() ? 
        ...
        assertTrue(latch.await(120, TimeUnit.SECONDS), "Stress test should complete within 2 minutes");
        executor.shutdown();

        // Verify system remained stable
        double averageResponseTime = (double) totalResponseTime.get() / (numberOfThreads * ordersPerThread);
        double successRate = (double) totalOrders.get() / (totalOrders.get() + totalErrors.get()) * 100;

        // Performance assertions
        assertTrue(maxResponseTime.get() < 30000, "No single order should take more than 30 seconds");
        assertTrue(averageResponseTime < 5000, "Average response time should be under 5 seconds");
        assertTrue(successRate > 30, "Success rate should be above 30% even under stress: " + successRate + "%");
        ...
        for (User user : multipleUsers) {
            ...
            long orderTransactions = userTransactions.stream()
                .filter(t -> t.getTransactionType() == TransactionType.ORDER_PAYMENT)
                .count();
            
            assertEquals(userOrders.size(), orderTransactions, 
                "Each order should have a corresponding transaction for user: " + updatedUser.getUsername());
        }

        log.info("Stress test completed - Total orders: {}, Total errors: {}, Success rate: {:.2f}%, " +
                "Max response time: {}ms, Average response time: {:.2f}ms", 
                totalOrders.get(), totalErrors.get(), successRate, maxResponseTime.get(), averageResponseTime);
    }
    ...
    void testOrderCreationWithDuplicateLinks() throws InterruptedException {
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    log.debug("Order rejected in thread {}: {}", threadIndex, e.getMessage());
                } finally {
                    latch.countDown();
                }
            });
        ...
        assertTrue(latch.await(60, TimeUnit.SECONDS), "Duplicate links test should complete within 60 seconds");
        executor.shutdown();
        ...
        assertTrue(successfulOrders.get() + rejectedOrders.get() == numberOfThreads, 
            "All requests should be accounted for");

        log.info("Duplicate links test completed - Successful: {}, Rejected: {}, " +
                "Orders with duplicate link: {}", 
                successfulOrders.get(), rejectedOrders.get(), ordersWithDuplicateLink.size());
    }
}