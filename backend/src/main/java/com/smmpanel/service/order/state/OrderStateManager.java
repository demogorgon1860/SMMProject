...
public class OrderStateManager {
    ...
    static {
        ...
        VALID_TRANSITIONS.put(OrderStatus.IN_PROGRESS, EnumSet.of(
        ...
        VALID_TRANSITIONS.put(OrderStatus.ACTIVE, EnumSet.of(
        ...
        VALID_TRANSITIONS.put(OrderStatus.PARTIAL, EnumSet.of(
            OrderStatus.COMPLETED, 
            OrderStatus.HOLDING
        ));
        
        VALID_TRANSITIONS.put(OrderStatus.PAUSED, EnumSet.of(
            OrderStatus.ACTIVE, 
            OrderStatus.CANCELLED
        ));
        
        VALID_TRANSITIONS.put(OrderStatus.HOLDING, EnumSet.of(
            OrderStatus.ACTIVE, 
            OrderStatus.CANCELLED, 
            OrderStatus.PROCESSING
        ));
        
        VALID_TRANSITIONS.put(OrderStatus.COMPLETED, EnumSet.of(
            OrderStatus.REFILL
        ));
        
        VALID_TRANSITIONS.put(OrderStatus.REFILL, EnumSet.of(
            OrderStatus.ACTIVE, 
            OrderStatus.COMPLETED
        ));
    ...
    public void transitionTo(Order order, OrderStatus newStatus) {
        ...
        if (!isValidTransition(currentStatus, newStatus)) {
            throw new IllegalOrderStateTransitionException(
                String.format("Invalid state transition from %s to %s for order %d", 
    ...
    private void logStateTransition(Long orderId, OrderStatus fromStatus, OrderStatus toStatus) {
        String message = String.format("Order %d state changed: %s -> %s", 
    ...
    private void publishStatusChangeEvent(Order order, OrderStatus oldStatus, OrderStatus newStatus) {
        try {
            ...
            log.error("Failed to publish status change event for order {}: {}", 
