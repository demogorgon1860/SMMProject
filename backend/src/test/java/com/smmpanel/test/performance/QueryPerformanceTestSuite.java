...
/**
 * Complete test suite for query performance testing Runs all performance tests and generates a
...
public class QueryPerformanceTestSuite {
    ...
    void runRepositoryOptimizationTests() {
        ...
        try {
            ...
            runTestAndCollectResult(
                    "Optimized Order Details Fetch",
                    () -> {
                        repoTests.testOptimizedOrderRepositoryFindWithDetails();
                    });

            runTestAndCollectResult(
                    "Optimized User Orders Pagination",
                    () -> {
    ...
    private static void generateMarkdownReport() throws IOException {
        try (FileWriter writer = new FileWriter(REPORT_FILE)) {
            ...
            long successfulTests = testResults.stream().filter(r -> r.getQueryCount() >= 0).count();
            ...
            if (failedTests > 0) {
                writer.write(
                        "⚠️ **Some tests failed - review implementation for performance"
                                + " issues**\n\n");
            } else {
            ...
            for (QueryPerformanceResult result : testResults) {
                ...
                String time =
                        result.getExecutionTimeMs() >= 0
                                ...
                                : "N/A";
                String notes =
                        result.getQueryCount() >= 0 ? "Performance within limits" : "Test failed";
            ...
            writer.write(
                    "1. **JOIN FETCH** - Prevent N+1 queries by fetching relationships in single"
                            + " query\n");
            ...
            writer.write(
                    "5. **Query Optimization** - Custom JPQL queries for specific use cases\n");
