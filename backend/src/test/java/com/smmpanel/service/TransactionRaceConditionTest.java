...
class TransactionRaceConditionTest {
    ...
    void testConcurrentBalanceCheckAndDeduct() throws InterruptedException {
        ...
        assertTrue(latch.await(30, TimeUnit.SECONDS), "Race condition test should complete within 30 seconds");
        executor.shutdown();
        ...
        assertEquals(10, successfulDeductions.get(), "Should have exactly 10 successful deductions (1000/100)");
        assertEquals(10, failedDeductions.get(), "Should have 10 failed deductions due to insufficient balance");
        
        // Final balance should be zero
        assertEquals(new BigDecimal("0.00"), updatedUser.getBalance());
        
        // Total spent should equal successful deductions
        BigDecimal expectedTotalSpent = deductionAmount.multiply(new BigDecimal(successfulDeductions.get()));
    ...
    void testConcurrentTransfers() throws InterruptedException {
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    failedTransfers.incrementAndGet();
                    System.err.println("Unexpected exception in transfer: " + e.getMessage());
                } finally {
                    latch.countDown();
                }
            });
        ...
        assertTrue(latch.await(45, TimeUnit.SECONDS), "Transfer race condition test should complete within 45 seconds");
        executor.shutdown();
    ...
    void testDeadlockPrevention() throws InterruptedException {
            ...
            executor.submit(() -> {
                ...
                try {
                    ...
                    balanceService.transferBalance(fromUser.getId(), toUser.getId(), 
                        transferAmount, "Deadlock prevention test " + threadIndex);
        ...
        assertTrue(latch.await(60, TimeUnit.SECONDS), "Deadlock prevention test should complete within 60 seconds");
        executor.shutdown();

        // Verify no deadlocks occurred (all operations completed within reasonable time)
        assertTrue(maxExecutionTime.get() < 30000, 
            "No single operation should take more than 30 seconds (indicating potential deadlock)");
        ...
        assertEquals(new BigDecimal("1000.00"), totalBalance, // 5 users * 200 each
    ...
    void testHighConcurrencyMixedOperations() throws InterruptedException {
        ...
        assertTrue(latch.await(120, TimeUnit.SECONDS), "Mixed operations test should complete within 2 minutes");
        executor.shutdown();
        ...
        assertTrue(finalUser1.getBalance().compareTo(BigDecimal.ZERO) >= 0, 
            "User balance should never go negative");
        assertTrue(finalUser2.getBalance().compareTo(BigDecimal.ZERO) >= 0, 
            "User balance should never go negative");
        ...
        System.out.printf("Mixed operations test completed: %d successful operations, %d exceptions%n", 
    ...
    void testTransactionTimeouts() throws InterruptedException {
                ...
                try {
                    ...
                    for (int j = 0; j < 10; j++) {
                        try {
                            ...
                            if (e.getMessage().contains("timeout") || e.getMessage().contains("deadlock")) {
        ...
        assertTrue(latch.await(90, TimeUnit.SECONDS), "Timeout test should complete within 90 seconds");
        executor.shutdown();
        ...
        System.out.printf("Timeout test completed: %d operations completed, %d timeouts detected%n", 
