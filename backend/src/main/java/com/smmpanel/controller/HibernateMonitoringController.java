...
/**
 * Controller for monitoring Hibernate performance and cache statistics Provides endpoints for
 * administrators to monitor database performance
 */
@RestController
@RequestMapping("/api/v1/admin/hibernate")
@RequiredArgsConstructor
@Tag(
        name = "Hibernate Monitoring",
        description = "Hibernate performance and cache monitoring endpoints")
...
public class HibernateMonitoringController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getHibernateStatistics() {
        try {
            ...
            return ResponseEntity.internalServerError()
                    .body(
                            ApiResponse.error(
                                    "Failed to retrieve Hibernate statistics: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getCacheStatistics() {
        ...
        try {
            ...
            return ResponseEntity.internalServerError()
                    .body(
                            ApiResponse.error(
                                    "Failed to retrieve cache statistics: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Object>> getQueryInspectorStatistics() {
        ...
        try {
            ...
            inspectorStats.put(
                    "slowQueryPercentage", String.format("%.2f%%", stats.getSlowQueryPercentage()));
            ...
            return ResponseEntity.internalServerError()
                    .body(
                            ApiResponse.error(
                                    "Failed to retrieve query inspector statistics: "
                                            + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<String>> logStatisticsSummary() {
        try {
            ...
            log.info(
                    "Second Level Cache Miss Count: {}", statistics.getSecondLevelCacheMissCount());
