...
public class OfferAssignmentController {
    ...
    @PostMapping("/assign")
    @Operation(
            summary = "Назначить оффер на фиксированные кампании",
            description =
                    "Создает оффер в Binom и назначает его на все 3 предварительно созданные"
                            + " кампании")
    ...
    public ResponseEntity<OfferAssignmentResponse> assignOfferSync(
        ...
        try {
            ...
            log.error(
                    "Sync offer assignment failed for order {}: {}",
                    request.getOrderId(),
                    e.getMessage(),
                    e);

            OfferAssignmentResponse errorResponse =
                    OfferAssignmentResponse.builder()
    ...
    @Operation(
            summary = "Асинхронное назначение оффера",
            description =
                    "Отправляет событие в Kafka для назначения оффера на фиксированные кампании")
    @PreAuthorize("hasRole('OPERATOR') or hasRole('ADMIN')")
    public ResponseEntity<String> assignOfferAsync(
        ...
        try {
            offerEventProducer.sendOfferAssignmentEvent(
                    request.getOrderId(), request.getOfferName(), request.getTargetUrl(), "MANUAL");

            return ResponseEntity.ok(
                    "Offer assignment event sent successfully for order: " + request.getOrderId());

        } catch (Exception e) {
            log.error(
                    "Failed to send async offer assignment for order {}: {}",
                    request.getOrderId(),
                    e.getMessage(),
                    e);
    ...
    @Operation(
            summary = "Получить назначенные кампании",
    ...
    public ResponseEntity<List<AssignedCampaignInfo>> getAssignedCampaigns(
        ...
        try {
            ...
            log.error(
                    "Failed to get assigned campaigns for order {}: {}",
                    orderId,
                    e.getMessage(),
                    e);
    ...
    @Operation(
            summary = "Быстрое назначение оффера",
    ...
    public ResponseEntity<String> quickAssignOffer(
            @RequestParam Long orderId, @RequestParam String targetUrl) {
        ...
        try {
            ...
            offerEventProducer.sendOfferAssignmentEvent(
                    orderId, offerName, targetUrl, "VIDEO_PROCESSING");

            log.info(
                    "Quick offer assignment initiated for order {} with URL: {}",
                    orderId,
                    targetUrl);
    ...
    @Operation(
            summary = "Проверить статус назначения",
    ...
    public ResponseEntity<String> getAssignmentStatus(@PathVariable Long orderId) {
        try {
            ...
            log.error(
                    "Failed to get assignment status for order {}: {}", orderId, e.getMessage(), e);
