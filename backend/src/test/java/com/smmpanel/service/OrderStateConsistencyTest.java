 ...
 * 
 * Tests state consistency across async operations with:
 * 1. Concurrent state transitions validation
 * 2. Race condition detection and prevention
 * 3. State consistency guarantees under high load
 * 4. Processing state cleanup and recovery
 * 5. Thread-safe access to shared state
 */
...
class OrderStateConsistencyTest {
    ...
    void testConcurrentValidationPreventsRaceConditions() throws InterruptedException {
        ...
        for (int i = 0; i < CONCURRENCY_THREADS; i++) {
            executor.submit(() -> {
                try {
                    startLatch.await(); // Synchronize start time to maximize race condition potential
                    
                    OrderValidationResult result = orderStateManagementService
                            .validateAndUpdateOrderForProcessing(TEST_ORDER_ID, TEST_VIDEO_ID);
        ...
        assertEquals(CONCURRENCY_THREADS - 1, failureCount.get(),
        ...
        assertEquals(TEST_VIDEO_ID, testOrder.getYoutubeVideoId(),
                "YouTube video ID should be set correctly");

        System.out.printf("CONCURRENT VALIDATION RESULTS:%n" +
    ...
    void testAtomicStateTransitions() throws InterruptedException {
        ...
        for (int i = 0; i < 5; i++) {
            executor.submit(() -> {
                try {
                    StateTransitionResult result = orderStateManagementService
                            .transitionToActive(TEST_ORDER_ID, 1000);
                    results.add(result);
        ...
        for (int i = 0; i < 5; i++) {
            executor.submit(() -> {
                try {
                    StateTransitionResult result = orderStateManagementService
                            .transitionToHolding(TEST_ORDER_ID, "Test transition");
                    results.add(result);
                    observedStates.add(testOrder.getStatus());
                } catch (Exception e) {
        ...
        assertTrue(finalStatus == OrderStatus.ACTIVE || finalStatus == OrderStatus.HOLDING,
                "Final status should be either ACTIVE or HOLDING: " + finalStatus);

        System.out.printf("ATOMIC TRANSITION RESULTS:%n" +
    ...
    void testThreadSafeProcessingStatusUpdates() throws InterruptedException {
        ...
        orderStateManagementService.validateAndUpdateOrderForProcessing(TEST_ORDER_ID, TEST_VIDEO_ID);
        ...
        OrderStateManagementService.ProcessingPhase[] phases = OrderStateManagementService.ProcessingPhase.values();
        ...
        Optional<ProcessingStateInfo> processingState = orderStateManagementService
                .getProcessingState(TEST_ORDER_ID);
        ...
        System.out.printf("PROCESSING STATUS UPDATE RESULTS:%n" +
                "- Total Updates: %d%n" +
    ...
    void testConcurrentProgressUpdates() throws InterruptedException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            executor.submit(() -> {
                try {
                    ProgressUpdateResult result = orderStateManagementService
                            .updateOrderProgress(TEST_ORDER_ID, currentViews);
                    results.add(result);
        ...
        if (completionCount.get() > 0) {
            ...
            assertEquals(0, testOrder.getRemains(),
                    "Remaining count should be 0 for completed orders");
        }

        System.out.printf("CONCURRENT PROGRESS UPDATE RESULTS:%n" +
                "- Total Progress Updates: %d%n" +
                "- Successful Updates: %d%n" +
                "- Completion Events: %d%n" +
                "- Final Status: %s%n" +
                "- Final Remains: %d%n",
                results.size(), successfulUpdates, completionCount.get(),
                testOrder.getStatus(), testOrder.getRemains());
    }
    ...
    void testStaleProcessingStateCleanup() throws InterruptedException {
        ...
        for (long i = 1; i <= 10; i++) {
            ...
            assertFalse(state.isPresent(), 
                    "Processing state should be cleaned up for order " + i);
        }
        ...
        for (Order order : testOrders.values()) {
            ...
            assertNotNull(order.getErrorMessage(),
                    "Order should have error message explaining timeout");
        }

        System.out.printf("CLEANUP TEST RESULTS:%n" +
                "- Cleanup Executions: %d%n" +
                "- Orders Processed: %d%n" +
                "- Orders in HOLDING: %d%n",
                cleanupExecutions.get(), testOrders.size(),
                testOrders.values().stream().mapToInt(o -> o.getStatus() == OrderStatus.HOLDING ? 1 : 0).sum());
    }
    ...
    void testInvalidStateTransitionRejection() throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            executor.submit(() -> {
                try {
                    ...
                    StateTransitionResult result = orderStateManagementService
                            .transitionToActive(TEST_ORDER_ID, 2000);
                    results.add(result);
                    ...
                    // Invalid transitions should be gracefully rejected, not throw exceptions
                    fail("Invalid transition should be rejected gracefully, not throw exception: " + e.getMessage());
                } finally {
        ...
        System.out.printf("INVALID TRANSITION REJECTION RESULTS:%n" +
                "- Total Attempts: %d%n" +
                "- Rejections: %d%n" +
                "- Final Status: %s%n" +
                "- Error Messages Present: %s%n",
                results.size(), rejectionCount.get(), testOrder.getStatus(),
