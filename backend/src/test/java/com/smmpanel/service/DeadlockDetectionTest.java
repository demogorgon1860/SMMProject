...
class DeadlockDetectionTest {
    ...
    void testCircularTransferDeadlockPrevention() throws InterruptedException {
            ...
            executor.submit(
                    () -> {
                        ...
                        try {
                            ...
                            failedTransfers.incrementAndGet();
                            log.debug(
                                    "Deadlock detected and handled in thread {}: {}",
                                    threadIndex,
        ...
        assertTrue(
                ...
                "Circular transfer test should complete within 3 minutes");
        executor.shutdown();

        // Verify results
        assertTrue(
                maxExecutionTime.get() < 60000,
                "No single operation should take more than 60 seconds (indicating deadlock"
                        + " resolution)");
        ...
        log.info(
                "Circular transfer test completed - Successful: {}, Failed: {}, Deadlocks detected:"
    ...
    void testMultiUserConcurrentOperationsDeadlockPrevention() throws InterruptedException {
        ...
        assertTrue(
                ...
                "Multi-user operations test should complete within 2 minutes");
        executor.shutdown();
        ...
        log.info(
                "Multi-user operations test completed - Operations: {}, Deadlock exceptions: {},"
                    + " Timeout exceptions: {}, Other exceptions: {}, Average wait time: {:.2f}ms",
                operations.get(),
                deadlockExceptions.get(),
        ...
        assertTrue(
                operations.get() > numberOfThreads * 2,
    ...
    void testHighContentionDeadlockRecovery() throws InterruptedException {
        ...
        assertTrue(
                ...
                "High contention test should complete within 90 seconds");
        executor.shutdown();

        // Verify balance conservation despite high contention
        BigDecimal totalFinalBalance =
                contentionUsers.stream()
                        .map(u -> userRepository.findById(u.getId()).orElseThrow().getBalance())
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        log.info(
                "High contention test completed - Successful: {}, Failed: {}, Retry attempts: {}, "
                        + "Exception types: {}",
        ...
        assertTrue(
                successfulOperations.get() > 0,
                "Should have some successful operations despite high contention");
    }
    ...
    void testDeadlockDetectionWithTimeouts() throws InterruptedException {
        ...
        assertTrue(
                ...
                "Timeout test should complete within 60 seconds");
        executor.shutdown();

        // Verify no operations took excessively long
        assertTrue(
                maxExecutionTime.get() < 45000,
                "No operation should take more than 45 seconds (indicating proper timeout"
                        + " handling)");

        log.info(
                "Timeout test completed - Completed operations: {}, Timeout operations: {}, "
    ...
    void testResourceLockOrderingDeadlockPrevention() throws InterruptedException {
        ...
        assertTrue(
                ...
                "Lock ordering test should complete within 150 seconds");
        executor.shutdown();
        ...
        BigDecimal expectedTotalBalance = new BigDecimal("8000.00"); // 8 users * 1000 each
        assertEquals(expectedTotalBalance, totalFinalBalance, "Total balance should be conserved");

        log.info(
                "Lock ordering test completed - Successful: {}, Failed: {}, Unique transfer"
                        + " directions: {}",
                successfulTransfers.get(),
                failedTransfers.get(),
                observedLockOrders.size());
    }
