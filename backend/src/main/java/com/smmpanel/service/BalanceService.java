...
public class BalanceService {
    ...
    @Retryable(value = { ObjectOptimisticLockingFailureException.class },
               ...
               backoff = @Backoff(delayExpression = "#{@balanceService.initialDelayMs}", 
    ...
    public void deductBalance(User user, BigDecimal amount, Order order, String description) {
        ...
        log.info("Deducted {} from user {}. New balance: {} (attempt: retry)", 
    ...
    public BigDecimal addBalance(User user, BigDecimal amount, BalanceDeposit deposit, String description) {
        ...
        log.info("Added {} to user {} balance. New balance: {}", amountToAdd, managedUser.getUsername(), newBalance);
    ...
    public void refund(User user, BigDecimal amount, Order order, String reason) {
        ...
        log.info("Refunded {} to user {} for order {}", refundAmount, managedUser.getUsername(), order != null ? order.getId() : "N/A");
    }
    ...
    @Retryable(value = { ObjectOptimisticLockingFailureException.class },
               ...
               backoff = @Backoff(delayExpression = "#{@balanceService.initialDelayMs}", 
                                 ...
                                 multiplierExpression = "#{@balanceService.backoffMultiplier}",
                                 random = true))
    public void transferBalance(Long fromUserId, Long toUserId, BigDecimal amount, String description) {
        ...
        if (fromCurrentBalance.compareTo(transferAmount) < 0) {
            throw new InsufficientBalanceException(
                String.format("Insufficient balance for transfer. Current: %s, Required: %s", 
        ...
        creditTransaction.setDescription(transferDescription + " from user " + fromUser.getUsername());
        creditTransaction.setCreatedAt(LocalDateTime.now());
        
        transactionRepository.save(debitTransaction);
        transactionRepository.save(creditTransaction);
        
        log.info("Transferred {} from user {} to user {}. New balances: {} -> {}, {} -> {}", 
            transferAmount, fromUser.getUsername(), toUser.getUsername(), 
            fromCurrentBalance, fromNewBalance, toCurrentBalance, toNewBalance);
    }
    ...
    @Retryable(value = { ObjectOptimisticLockingFailureException.class },
               ...
               backoff = @Backoff(delayExpression = "#{@balanceService.initialDelayMs}", 
                                 ...
                                 multiplierExpression = "#{@balanceService.backoffMultiplier}",
                                 random = true))
    public BigDecimal adjustBalance(Long userId, BigDecimal adjustment, TransactionType transactionType, 
        ...
        if (adjustmentAmount.compareTo(BigDecimal.ZERO) < 0 && newBalance.compareTo(BigDecimal.ZERO) < 0) {
            throw new InsufficientBalanceException(
                String.format("Insufficient balance for adjustment. Current: %s, Adjustment: %s", 
        ...
        log.info("Adjusted balance for user {} by {}. New balance: {} (transaction type: {})", 
            managedUser.getUsername(), adjustmentAmount, newBalance, transactionType);
        
        return newBalance;
    }
    
    /**
     * Checks and reserves balance atomically for an order.
     * This method checks if sufficient balance exists and temporarily holds it.
     * @param userId User ID
    ...
    public boolean checkAndReserveBalance(Long userId, BigDecimal amount) {
        ...
        log.debug("Balance check for user {}: current={}, required={}, sufficient={}", 
            managedUser.getUsername(), currentBalance, reserveAmount, hasSufficientBalance);
        
        return hasSufficientBalance;
    }
    
    /**
     * Atomically checks balance and deducts if sufficient (prevents race conditions).
     * This method combines balance check and deduction in a single atomic operation.
    ...
    @Retryable(value = { ObjectOptimisticLockingFailureException.class },
               ...
               backoff = @Backoff(delayExpression = "#{@balanceService.initialDelayMs}", 
                                 ...
                                 multiplierExpression = "#{@balanceService.backoffMultiplier}",
                                 random = true))
    public boolean checkAndDeductBalance(User user, BigDecimal amount, Order order, String description) {
        Objects.requireNonNull(user, "User cannot be null");
        Objects.requireNonNull(amount, "Amount cannot be null");
        validateAmount(amount);
        ...
        if (currentBalance.compareTo(amountToDeduct) < 0) {
            log.debug("Insufficient balance for user {}: current={}, required={}", 
                managedUser.getUsername(), currentBalance, amountToDeduct);
            return false;
        }
        ...
        log.info("Successfully checked and deducted {} from user {}. New balance: {}", 
            amountToDeduct, managedUser.getUsername(), newBalance);
    ...
    /**
     * Executes a balance operation within a programmatic transaction with custom isolation.
     * This method provides fine-grained control over transaction properties.
     * @param operation The operation to execute
     * @param <T> Return type of the operation
     * @return Result of the operation
     */
    public <T> T executeInBalanceTransaction(Supplier<T> operation) {
        return balanceTransactionTemplate.execute(status -> {
            try {
                return operation.get();
            } catch (Exception e) {
                status.setRollbackOnly();
                log.error("Balance transaction failed, marking for rollback: {}", e.getMessage());
                throw e;
            }
        });
    }
