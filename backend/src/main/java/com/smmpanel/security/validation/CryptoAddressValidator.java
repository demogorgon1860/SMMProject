 ...
 *
 * <p>SECURITY FEATURES: - Multi-currency address format validation - Checksum verification for
 * Bitcoin and derivatives - Ethereum address checksum validation (EIP-55) - Testnet address
 * detection and blocking - Known malicious address blacklist checking - Address type validation
 * (Legacy, SegWit, Bech32)
 */
@Slf4j
@Component
public class CryptoAddressValidator implements ConstraintValidator<CryptoAddress, String> {
    ...
    private static final Pattern BTC_LEGACY_PATTERN =
            Pattern.compile("^[13][a-km-zA-HJ-NP-Z1-9]{25,34}$");
    private static final Pattern BTC_SEGWIT_PATTERN =
    ...
    private static final Pattern BTC_TESTNET_PATTERN =
            Pattern.compile("^[2mn][a-km-zA-HJ-NP-Z1-9]{25,34}$|^tb1[a-z0-9]{39,59}$");

    // Ethereum address pattern
    private static final Pattern ETH_ADDRESS_PATTERN = Pattern.compile("^0x[a-fA-F0-9]{40}$");
    private static final Pattern ETH_TESTNET_PATTERN =
            Pattern.compile("^0x[a-fA-F0-9]{40}$"); // Same format, context-dependent

    // Litecoin address patterns
    private static final Pattern LTC_LEGACY_PATTERN =
            Pattern.compile("^[LM][a-km-zA-HJ-NP-Z1-9]{26,33}$");
    private static final Pattern LTC_SEGWIT_PATTERN = Pattern.compile("^ltc1[a-z0-9]{39,59}$");
    private static final Pattern LTC_TESTNET_PATTERN =
            Pattern.compile("^[mn2][a-km-zA-HJ-NP-Z1-9]{25,34}$|^tltc1[a-z0-9]{39,59}$");

    // Known malicious address patterns (example - would be loaded from external source)
    private static final Set<String> KNOWN_MALICIOUS_ADDRESSES =
            Set.of(
                    // Example malicious addresses - in production, load from threat intelligence
                    // feed
    ...
    public boolean isValid(String address, ConstraintValidatorContext context) {
        ...
        if (detectedCurrency == null) {
            addConstraintViolation(
                    context, "Unable to determine cryptocurrency type from address format");
            return false;
        }
        ...
        if (!isCurrencySupported(detectedCurrency)) {
            addConstraintViolation(
                    context,
                    String.format(
                            "%s addresses are not supported", detectedCurrency.getDisplayName()));
            return false;
        }
    ...
    private boolean validateCurrencySpecificFormat(
            String address,
    ...
    private boolean validateEthereumChecksum(String address) {
        try {
            ...
            boolean hasMixedCase =
                    !hexPart.equals(hexPart.toLowerCase())
    ...
    private boolean isKnownMaliciousAddress(String address) {
        if (KNOWN_MALICIOUS_ADDRESSES.contains(address)) {
            log.warn(
                    "Known malicious address detected: {}",
        ...
        if (isSuspiciousAddressPattern(address)) {
            log.warn(
                    "Suspicious address pattern detected: {}",
                    address.substring(0, Math.min(address.length(), 10)) + "...");
            return true;
        }
    ...
    private boolean isSuspiciousAddressPattern(String address) {
        ...
        if (lowerAddress.contains("test")
                || lowerAddress.contains("example")
                || lowerAddress.contains("1111")
                || lowerAddress.contains("0000")) {
            return true;
        }
