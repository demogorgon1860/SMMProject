...
/**
 * Custom Hibernate Statement Inspector for monitoring and performance analysis
 * Tracks query execution patterns, slow queries, and provides detailed statistics
 */
@Component
@Slf4j
public class HibernateStatementInspector implements StatementInspector {
    ...
    private final ConcurrentHashMap<String, AtomicLong> queryTypeCounters = new ConcurrentHashMap<>();
    ...
    private void analyzeQueryForIssues(String sql, String queryType) {
        ...
        if ("SELECT".equals(queryType) && normalizedSql.contains("WHERE") && 
            !normalizedSql.contains("JOIN") && normalizedSql.contains("IN (")) {
            log.warn("Potential N+1 query pattern detected: {}", formatSqlForLogging(sql));
        }
        
        // Check for missing pagination
        if ("SELECT".equals(queryType) && !normalizedSql.contains("LIMIT") && 
            !normalizedSql.contains("ROWNUM") && !normalizedSql.contains("TOP")) {
        ...
        if ("SELECT".equals(queryType) && normalizedSql.contains("SELECT *")) {
            log.warn("SELECT * query detected (consider selecting specific columns): {}", 
        ...
        if (normalizedSql.contains("WHERE") && normalizedSql.contains("LIKE '%")) {
            log.warn("LIKE query with leading wildcard detected (may be slow): {}", 
                formatSqlForLogging(sql));
        }
    }
    ...
    public void logStatisticsSummary() {
        ...
        log.info("Slow Queries: {} ({}%)", stats.getTotalSlowQueries(), 
            stats.getTotalQueries() > 0 ? 
                String.format("%.2f", (double) stats.getTotalSlowQueries() / stats.getTotalQueries() * 100) : "0");
        
        log.info("Query Type Breakdown:");
        stats.getQueryTypeCounters().forEach((type, count) -> 
            log.info("  {}: {} ({}%)", type, count.get(), 
                String.format("%.2f", (double) count.get() / stats.getTotalQueries() * 100)));
