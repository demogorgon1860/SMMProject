...
class CurrencyUtilsTest {
    ...
    void setUp() {
        ...
        when(currencyService.getSupportedCurrencies())
                .thenReturn(Map.of(
                        ...
                        "BHD", new CurrencyService.CurrencyInfo(".د.ب", 3, "Bahraini Dinar", false)
                ));
    ...
    void formatCurrency_USD_FormatsCorrectly() {
        String result = CurrencyUtils.formatCurrency(new BigDecimal("1234.567"), "USD", currencyService);
        assertEquals("$1,234.57", result);
    }

    @Test
    void formatCurrency_EUR_FormatsWithSymbolAfter() {
        String result = CurrencyUtils.formatCurrency(new BigDecimal("1234.5"), "EUR", currencyService);
        assertEquals("1,234.50 €", result);
    }

    @Test
    void formatCurrency_JPY_RoundsToNoDecimals() {
        String result = CurrencyUtils.formatCurrency(new BigDecimal("1234.56"), "JPY", currencyService);
        assertEquals("¥1,235", result);
    }

    @Test
    void formatCurrency_BHD_ThreeDecimalPlaces() {
        String result = CurrencyUtils.formatCurrency(new BigDecimal("1234.5678"), "BHD", currencyService);
    ...
    void convertAmount_ConvertsCorrectly() {
        ...
        BigDecimal result = CurrencyUtils.convertAmount(amount, exchangeRate, "EUR", currencyService);
    ...
    void validatePositiveAmount_ValidAmount_NoException() {
        assertDoesNotThrow(() -> CurrencyUtils.validatePositiveAmount(new BigDecimal("100"), "amount"));
    }
