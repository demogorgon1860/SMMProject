...
/**
 * Service to handle optimistic locking conflicts with retry mechanism Provides utilities for
 * handling concurrent entity modifications
 */
@Service
...
public class OptimisticLockingService {
    ...
    public <T> T executeWithRetry(Supplier<T> operation, String entityType, Object entityId) {
        try {
            ...
            log.warn(
                    "Optimistic locking failure for {} with ID: {}. Retrying...",
                    entityType,
                    entityId);
     ...
     * @throws OptimisticLockingException if all retry attempts fail
     */
    public <T> T executeWithManualRetry(
            Supplier<T> operation, int maxRetries, String entityType, Object entityId) {
        int attempt = 0;
        ...
        while (attempt < maxRetries) {
            try {
                log.debug(
                        "Executing operation for {} with ID: {} (attempt {}/{})",
                        entityType,
                        entityId,
                        attempt + 1,
                        maxRetries);
                ...
                if (attempt < maxRetries) {
                    ...
                    log.warn(
                            "Optimistic locking failure for {} with ID: {} (attempt {}/{}). "
                                    + "Retrying in {}ms...",
                            entityType,
                            entityId,
                            attempt,
                    ...
                    try {
                        ...
                        throw new OptimisticLockingException(
                                "Retry interrupted for " + entityType + " with ID: " + entityId,
                                ie);
                    }
                } else {
                    log.error(
                            "All retry attempts failed for {} with ID: {} after {} attempts",
                            entityType,
                            entityId,
                            maxRetries);
                }
     ...
     */
    public String createConflictInfo(
            String entityType,
            Object entityId,
