...
/**
 * Minimal but powerful test flow to verify Binom API integration Tests: Campaign creation ->
 * Click/Conversion events -> HTTP 200 verification -> System consistency
 */
...
public class BinomApiSimpleFlowTest {
    ...
    void testCompleteBinomApiFlow() {
        ...
        when(binomClient.assignOfferToCampaign(anyString(), eq(OFFER_ID)))
                .thenReturn(
                        AssignOfferResponse.builder().status("ASSIGNED").offerId(OFFER_ID).build());
    ...
    void testHttp200ResponseVerification() {
        ...
        CampaignStatsResponse statsResponse =
    ...
    void testDatabaseKafkaConsistency() {
        ...
        when(binomClient.checkOfferExists(anyString()))
                .thenReturn(CheckOfferResponse.builder().exists(true).offerId(OFFER_ID).build());
        ...
        for (BinomCampaign campaign : savedCampaigns) {
            assertEquals(
                    ...
                    campaign.getCoefficient(),
                    "Without clip, coefficient should be 4.0");
        }

        // Verify total clicks with coefficient 4.0
        int totalClicks = savedCampaigns.stream().mapToInt(BinomCampaign::getClicksRequired).sum();
    ...
    private void testCampaignStatsAggregation(List<BinomCampaign> campaigns) {
        ...
        when(binomCampaignRepository.findByOrderIdAndActiveTrue(ORDER_ID)).thenReturn(campaigns);

        // Mock stats for each campaign
        when(binomClient.getCampaignStats("FIXED_CAMP_001"))
                .thenReturn(createStats("FIXED_CAMP_001", 100L, 20L));
        when(binomClient.getCampaignStats("FIXED_CAMP_002"))
                .thenReturn(createStats("FIXED_CAMP_002", 200L, 40L));
        when(binomClient.getCampaignStats("FIXED_CAMP_003"))
                .thenReturn(createStats("FIXED_CAMP_003", 300L, 60L));
        ...
        assertTrue(
                aggregatedStats.getCampaignId().contains(","), "Should have comma-separated IDs");
