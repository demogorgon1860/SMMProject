...
class CurrencyConversionServiceTest {
    ...
    private final User testUser =
            User.builder().id(1L).username("testuser").preferredCurrency("EUR").build();
    ...
    void setUp() {
        ...
        when(exchangeRateService.getExchangeRate("EUR", "USD")).thenReturn(new BigDecimal("1.18"));
    ...
    void convert_SameCurrency_ReturnsSameAmount() {
        CurrencyConversionRequest request =
                new CurrencyConversionRequest(new BigDecimal("100"), "USD", "USD", false);
    ...
    void convert_DifferentCurrencies_ReturnsConvertedAmount() {
        CurrencyConversionRequest request =
                new CurrencyConversionRequest(new BigDecimal("100"), "USD", "EUR", false);
    ...
    void convert_UnsupportedCurrency_ThrowsException() {
        CurrencyConversionRequest request =
                new CurrencyConversionRequest(new BigDecimal("100"), "USD", "XYZ", false);
    ...
    void formatCurrency_ValidInput_ReturnsFormattedString() {
        ...
        String result = currencyConversionService.formatCurrency(new BigDecimal("1234.56"), "USD");
    ...
    void formatForUser_ValidInput_ReturnsFormattedString() {
        when(currencyService.formatCurrency(new BigDecimal("100"), "EUR")).thenReturn("100,00 €");

        String result = currencyConversionService.formatForUser(new BigDecimal("100"), testUser);
    ...
    void convertAndFormatForUser_SameCurrency_ReturnsFormattedAmount() {
        ...
        String result =
                currencyConversionService.convertAndFormatForUser(
    ...
    void convertAndFormatForUser_DifferentCurrencies_ReturnsConvertedAndFormatted() {
        when(currencyService.formatCurrency(new BigDecimal("85.00"), "EUR")).thenReturn("85,00 €");
