...
/**
 * Comprehensive tests for Hibernate second-level cache effectiveness Validates that caching is
 * working correctly and improving performance
 */
@SpringBootTest
@ActiveProfiles("test")
@Slf4j
public class HibernateSecondLevelCacheTest {
    ...
    public void testEntitySecondLevelCache() {
        ...
        log.info(
                "First access - Cache hits: {}, misses: {}, queries: {}",
        ...
        log.info(
                "Second access - Cache hits: {}, misses: {}, queries: {}",
        ...
        if (secondAccessHits > firstAccessHits) {
            ...
            assertEquals(
                    firstAccessQueries,
                    secondAccessQueries,
                    "Second access should not execute additional queries");
        } else {
            log.warn("⚠️ Second-level cache may not be working as expected");
            log.warn(
                    "This could be due to test configuration or cache not being enabled for Service"
                            + " entity");
        }
    }
    ...
    public void testQueryCacheEffectiveness() {
        ...
        log.info(
                "First query - Cache hits: {}, misses: {}, queries: {}",
        ...
        log.info(
                "Second query - Cache hits: {}, misses: {}, queries: {}",
        ...
        // Validate results
        assertEquals(
                firstQueryResult.size(),
                secondQueryResult.size(),
    ...
    public void testCachePerformanceImpact() {
        ...
        log.info(
                "First run (cold cache): {}ms, {} queries, {} cache hits",
                firstRunTime,
                firstRunQueries,
                firstRunCacheHits);
        ...
        log.info(
                "Second run (warm cache): {}ms, {} queries, {} cache hits",
                secondRunTime,
                secondRunQueries,
                secondRunCacheHits);
        ...
        if (secondRunCacheHits > 0) {
            double performanceImprovement =
                    ((double) (firstRunTime - secondRunTime) / firstRunTime) * 100;
