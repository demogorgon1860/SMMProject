 ...
 *
 * <p>Validates that the async processing configuration is working correctly: 1. Thread pool
 * executors are properly configured 2. Tasks execute asynchronously in dedicated thread pools 3.
 * Thread pool sizing and queue management works as expected 4. Health indicators provide accurate
 * metrics
 */
...
class AsyncVideoProcessingConfigTest {
    ...
    @Autowired private VideoProcessingHealthIndicator healthIndicator;

    @Autowired private AsyncThreadPoolMonitor threadPoolMonitor;
    ...
    void testVideoProcessingExecutorConfiguration() {
        ...
        assertTrue(
                videoProcessingExecutor instanceof ThreadPoolTaskExecutor,
        ...
        assertTrue(
                executor.getThreadNamePrefix().contains("VideoProcessing"),
    ...
    void testLightweightExecutorConfiguration() {
        ...
        assertTrue(
                lightweightAsyncExecutor instanceof ThreadPoolTaskExecutor,
                "Should be ThreadPoolTaskExecutor instance");
        ...
        assertTrue(
                executor.getThreadNamePrefix().contains("Lightweight"),
    ...
    void testAsyncTaskExecution() throws InterruptedException {
            ...
            CompletableFuture.runAsync(
                    () -> {
                        try {
                            ...
                            assertTrue(
                                    workerThreadName.contains("VideoProcessing"),
                                    "Task should execute in VideoProcessing thread: "
                                            + workerThreadName);
        ...
        assertTrue(
                executor.getThreadPoolExecutor().getTaskCount() >= 5,
    ...
    void testConcurrentTaskHandling() throws InterruptedException {
        ...
        System.out.printf(
                "Concurrent Task Test Results:%n"
    ...
    void testThreadPoolMonitorMetrics() throws InterruptedException {
        ...
        assertTrue(finalMetrics.getTotalTasks() >= 4, "Should have processed at least 4 tasks");

        System.out.printf(
                "Thread Pool Metrics:%n" + "- Video Processing: %s%n" + "- Lightweight Async: %s%n",
    ...
    void testTaskFailureRecovery() throws InterruptedException {
        ...
        assertTrue(completedTasks >= 6, "Should have completed all tasks despite failures");

        System.out.printf(
                "Failure Recovery Test Results:%n"
