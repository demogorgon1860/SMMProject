 ...
 * field). This allows users to see real-time delivery progress instead of static values.
 */
@Slf4j
@Component
...
public class InstagramProgressScheduler {
    ...
    public void updateInstagramOrderProgress() {
        ...
        for (Order order : instagramOrders) {
            try {
                InstagramOrderStatus botStatus =
                        instagramBotClient.getOrderStatus(order.getInstagramBotOrderId());
                ...
                log.warn("Failed to poll progress for order {}: {}", order.getId(), e.getMessage());
            }
        }
