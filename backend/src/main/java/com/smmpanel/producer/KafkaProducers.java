...
public class KafkaProducers {
        ...
        try {
            ...
            future.whenComplete(
                    (result, ex) -> {
                        if (ex == null) {
                            log.info(
                                    "Successfully sent video processing request: {} with offset:"
                                            + " {}",
                                    processingId,
                                    result.getRecordMetadata().offset());
                        } else {
                            log.error(
                                    "Failed to send video processing request: {}",
                                    processingId,
                                    ex);
                        }
                    });
        ...
        try {
            ...
            future.whenComplete(
                    (result, ex) -> {
                        if (ex == null) {
                            log.info(
                                    "Successfully sent offer assignment request for order: {} with"
                                            + " offset: {}",
                                    request.getOrderId(),
                                    result.getRecordMetadata().offset());
                        } else {
                            log.error(
                                    "Failed to send offer assignment request for order: {}",
                                    request.getOrderId(),
                                    ex);
                        }
                    });
    ...
    public void sendOrderStateUpdate(Long orderId, String oldStatus, String newStatus) {
        try {
            String message =
                    String.format(
                            "{\"orderId\":%d,\"oldStatus\":\"%s\",\"newStatus\":\"%s\",\"timestamp\":\"%s\"}",
