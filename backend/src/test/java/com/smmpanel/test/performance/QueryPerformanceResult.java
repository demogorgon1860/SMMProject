...
/**
 * Performance measurement result for database operations Contains detailed metrics about query
...
public class QueryPerformanceResult {
    ...
    public static class PerformanceComparison {
        ...
        public boolean isRegression() {
            return queryCountDelta > 0
                    || executionTimeDelta > currentResult.executionTimeMs * 0.2; // 20% slower
        }

        public String getSummary() {
            return String.format(
                    "Performance comparison: Queries %+d, Time %+dms (%.1f%%), Cache Hit Ratio"
                            + " %+.1f%%",
                    queryCountDelta,
                    executionTimeDelta,
                    currentResult.executionTimeMs > 0
                            ? (double) executionTimeDelta / currentResult.executionTimeMs * 100
                            : 0,
