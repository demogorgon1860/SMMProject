...
class DataConsistencyVerificationTest {
    ...
    void testDataConsistencyAfterIntensiveConcurrentOperations() throws InterruptedException {
                                ...
                                try {
                                    ...
                                    switch ((threadIndex + j) % 5) {
                                        ...
                                        case 1: // Debit
                                            Order order =
                                                    createTestOrder(
                                                            randomUser, new BigDecimal("3.00"));
                                            boolean deducted =
                                                    balanceService.checkAndDeductBalance(
                                                            randomUser,
                                                            new BigDecimal("3.00"),
                                                            order,
                                                            "Consistency test debit");
                                            if (deducted) {
        ...
        assertTrue(
                ...
                "Consistency test should complete within 3 minutes");
        executor.shutdown();
        ...
        log.info(
                "Intensive consistency test completed - Total: {}, Successful: {}, Failed: {}, "
                        + "Operation counts: {}",
                totalOperations.get(),
    ...
    void testBalanceTransactionIntegrity() throws InterruptedException {
        ...
        assertTrue(
                ...
                "Integrity test should complete within 2 minutes");
        executor.shutdown();
        ...
        for (BalanceTransaction transaction : allTransactions) {
            ...
            BigDecimal expectedBalanceAfter =
                    transaction.getBalanceBefore().add(transaction.getAmount());
            assertEquals(
                    expectedBalanceAfter,
                    transaction.getBalanceAfter(),
                    "BalanceAfter should equal BalanceBefore + Amount for transaction "
                            + transaction.getId());
        }

        log.info(
                "Integrity test completed - Total transactions: {}, No duplicates found, All"
    ...
    void testUserBalanceTotalSpentConsistency() throws InterruptedException {
        ...
        assertTrue(
                ...
                "TotalSpent consistency test should complete within 90 seconds");
        executor.shutdown();
        ...
        for (User originalUser : testUsers) {
            ...
            List<BalanceTransaction> userTransactions =
                    transactionRepository.findByUserId(originalUser.getId(), null).getContent();

            BigDecimal expectedTotalSpent =
                    userTransactions.stream()
                            .filter(
                                    t ->
                                            t.getAmount().compareTo(BigDecimal.ZERO)
                                                    < 0) // Negative amounts are debits
                            .map(t -> t.getAmount().abs())
                            .reduce(BigDecimal.ZERO, BigDecimal::add);

            assertEquals(
                    expectedTotalSpent,
                    updatedUser.getTotalSpent(),
                    "TotalSpent should match sum of debit transactions for user: "
    ...
    void testOrderBalanceTransactionConsistency() throws InterruptedException {
        ...
        assertTrue(
                latch.await(120, TimeUnit.SECONDS),
                "Order consistency test should complete within 2 minutes");
        executor.shutdown();
        ...
        Map<Long, List<BalanceTransaction>> transactionsByOrder =
        ...
        log.info(
                "Order consistency test completed - Orders: {}, All order-transaction relationships"
                        + " verified",
                allOrders.size());
    }
    ...
    void testConcurrentBalanceModificationDataRaces() throws InterruptedException {
        ...
        AtomicReference<BigDecimal> observedMinBalance =
                new AtomicReference<>(testUser.getBalance());
        AtomicReference<BigDecimal> observedMaxBalance =
                new AtomicReference<>(testUser.getBalance());
                            ...
                            for (int j = 0; j < 5; j++) {
                                try {
                                    if (j % 2 == 0) {
                                        ...
                                        Order order =
                                                createTestOrder(testUser, new BigDecimal("2.00"));
                                        balanceService.checkAndDeductBalance(
                                                testUser,
                                                new BigDecimal("2.00"),
                                                order,
                                    ...
                                    observedMinBalance.accumulateAndGet(
                                            currentBalance,
                                            (current, observed) ->
                                                    current.compareTo(observed) < 0
                                                            ? current
                                                            : observed);
                                    observedMaxBalance.accumulateAndGet(
                                            currentBalance,
                                            (current, observed) ->
                                                    current.compareTo(observed) > 0
                                                            ? current
        ...
        assertTrue(
                ...
                "Data race test should complete within 60 seconds");
        executor.shutdown();
    ...
    private void verifyDataConsistency(Map<String, AtomicInteger> operationCounts) {
        ...
        BigDecimal finalTotalBalance =
                testUsers.stream()
                        .map(u -> userRepository.findById(u.getId()).orElseThrow().getBalance())
                        .reduce(BigDecimal.ZERO, BigDecimal::add);

        // Calculate expected balance change
        BigDecimal expectedCredits =
                new BigDecimal("5.00")
                        .multiply(new BigDecimal(operationCounts.get("credits").get()));
        BigDecimal expectedDebits =
                new BigDecimal("3.00")
                        .multiply(new BigDecimal(operationCounts.get("debits").get()));
        BigDecimal expectedRefunds =
                new BigDecimal("1.25")
                        .multiply(new BigDecimal(operationCounts.get("refunds").get()));

        // Adjustments are variable, so we'll calculate from actual transactions
        List<BalanceTransaction> adjustmentTransactions =
                transactionRepository.findAll().stream()
                        .filter(t -> t.getTransactionType() == TransactionType.ADJUSTMENT)
                        .toList();

        BigDecimal actualAdjustments =
        ...
        assertTrue(
                Math.abs(expectedTotalBalance.subtract(finalTotalBalance).doubleValue()) < 0.01,
                String.format(
                        "Total balance conservation failed. Expected: %s, Actual: %s, Difference:"
                                + " %s",
                        expectedTotalBalance,
                        finalTotalBalance,
                        expectedTotalBalance.subtract(finalTotalBalance)));
        ...
        long expectedTransactionCount =
                operationCounts.values().stream().mapToLong(AtomicInteger::get).sum();

        // Allow some variance due to failed operations
        assertTrue(
                allTransactions.size() <= expectedTransactionCount,
        ...
        log.info(
                "Data consistency verification passed - Total balance: {}, Transactions: {}, "
