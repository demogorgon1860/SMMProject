...
/**
 * Performance tests with different data volumes to ensure scalability
 * Tests how query performance changes with increasing data size
 */
@Slf4j
public class DataVolumePerformanceTest extends QueryPerformanceTestBase {
    ...
    public void testOrderListingWithVariableUserCounts(int userCount) {
        ...
        QueryPerformanceResult result = measureQueryPerformance(
            "Order Listing (" + userCount + " users)", 
            () -> {
                Page<Order> userOrders = orderRepository.findOrdersWithDetailsByUserId(
                    testUser.getId(), 
                    pageable
                );
        ...
        assert result.getExecutionTimeMs() < maxTimeMs : 
            String.format("Order listing with %d users took %dms, expected < %dms", 
    ...
    public void testServiceListingWithVariableServiceCounts(int serviceCount) {
        ...
        assert result.getExecutionTimeMs() < maxTimeMs :
            String.format("Service listing with %d services took %dms, expected < %dms",
                serviceCount, result.getExecutionTimeMs(), maxTimeMs);
        
        log.info("✓ Service listing performance acceptable with {} services: {}", serviceCount, result);
    }
    ...
    public void testOrderSearchWithVariableOrderCounts(int ordersPerUser) {
        ...
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                for (Order order : activeOrders.subList(0, Math.min(10, activeOrders.size()))) {
                    order.getUser().getUsername();
                    order.getService().getName();
                }
            }
        ...
        assert result.getExecutionTimeMs() < maxTimeMs :
            String.format("Order search with %d orders took %dms, expected < %dms",
                ordersPerUser * users.size(), result.getExecutionTimeMs(), maxTimeMs);
        
        log.info("✓ Order search performance acceptable with {} total orders: {}", 
            ordersPerUser * users.size(), result);
    }
    ...
    public void testPerformanceComparisonSmallVsLargeDataset() {
        ...
        QueryPerformanceResult largeDatasetResult = measureQueryPerformance(
            "Large Dataset Order Listing",
            () -> {
                Pageable pageable = PageRequest.of(0, 10);
                Page<Order> orders = orderRepository.findOrdersWithDetailsByUserId(
                    largeUsers.get(0).getId(), 
                    pageable
                );
        ...
        assert largeDatasetResult.getExecutionTimeMs() <= smallDatasetResult.getExecutionTimeMs() * 3 :
            String.format("Large dataset performance degradation too high: %dms vs %dms",
                largeDatasetResult.getExecutionTimeMs(), smallDatasetResult.getExecutionTimeMs());
        
        log.info("✓ Performance degradation acceptable: Small={}ms, Large={}ms", 
            smallDatasetResult.getExecutionTimeMs(), largeDatasetResult.getExecutionTimeMs());
    }
    ...
    public void testBatchLoadingPerformanceWithDifferentSizes() {
        ...
        for (int batchSize : batchSizes) {
            ...
            assert result.getExecutionTimeMs() < maxTimeMs :
                String.format("Batch loading %d orders took %dms, expected < %dms",
    ...
    public void testPerformanceUnderMemoryPressure() {
        ...
        assert result.getExecutionTimeMs() < 1000 : "Operations under memory pressure should complete in under 1 second";
