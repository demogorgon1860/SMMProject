 ...
 *
 * <p>Tests the async processing setup with realistic load scenarios: 1. High concurrent order
 * processing (50+ simultaneous orders) 2. Thread pool saturation and recovery 3. Queue management
 * under pressure 4. Resource cleanup and memory management 5. Performance degradation detection
 */
...
class AsyncVideoProcessingLoadTest {
    ...
    @MockBean private VideoProcessingRepository videoProcessingRepository;

    @MockBean private YouTubeAccountRepository youTubeAccountRepository;
    ...
    @MockBean private BinomService binomService;
    ...
    @MockBean private OrderStateManagementService orderStateManagementService;
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(
                youTubeAutomationService, "clipCreationTimeoutMs", 5000L); // Shorter for testing
    }
    ...
    void testHighConcurrentOrderProcessing() throws InterruptedException {
        ...
        assertTrue(
                successCount.get() >= HIGH_LOAD_ORDER_COUNT * 0.95,
                "At least 95% of orders should succeed. Success: " + successCount.get());
        assertTrue(
                errorCount.get() <= HIGH_LOAD_ORDER_COUNT * 0.05,
    ...
    void testThreadPoolSaturationRecovery() throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            maxQueueSize =
                    Math.max(maxQueueSize, executor.getThreadPoolExecutor().getQueue().size());
        }
    ...
    void testPerformanceDegradationDetection() throws InterruptedException {
        ...
        assertTrue(processingTimes.size() >= 25, "Should have timing data for most orders");
        ...
        assertTrue(
                p95Time < 15000, "95th percentile should be under 15s. Actual: " + p95Time + "ms");
        assertTrue(
                avgTime < 8000,
    ...
    void testErrorRecoveryUnderLoad() throws InterruptedException {
        ...
        assertTrue(successfulOrders.get() > 0, "Some orders should succeed despite errors");
        assertTrue(failedOrders.get() > 0, "Some orders should fail as expected");
    ...
    private void setupMocksForLoadTest() {
        ...
        when(binomService.createBinomIntegration(any()))
                .thenReturn(createSuccessfulBinomResponse());
    }
    ...
    private void setupSlowMocksForSaturationTest() {
        ...
        when(youTubeService.getVideoViewCount(anyString()))
                .thenAnswer(
                        invocation -> {
                            try {
                        ...
                        invocation -> {
                            ...
                            if (orderId % 3 == 0) {
                                throw new RuntimeException(
                                        "Simulated database error for order " + orderId);
                            }
                            return Optional.of(createTestOrder(orderId));
                        });
    ...
    private Order createTestOrder(Long orderId) {
        ...
        String videoId =
                String.format("abc%08d", orderId); // Creates abc00000001, abc00000002, etc.
