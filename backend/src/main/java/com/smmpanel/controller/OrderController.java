 ...
 *
 * <p>IMPROVEMENTS: 1. Added comprehensive input validation with @Valid 2. Implemented rate limiting
 * per user 3. Added proper API documentation with OpenAPI 4. Security annotations for authorization
 * 5. Perfect Panel compatible response format 6. Proper error handling and logging
 */
@Slf4j
@RestController
...
public class OrderController {
    ...
    public ResponseEntity<PerfectPanelResponse<?>> createOrder(
        ...
        PerfectPanelResponse<OrderResponse> response =
                PerfectPanelResponse.<OrderResponse>builder()
                        .data(order)
                        .success(true)
    ...
    /**
     * Get user's orders with pagination and filtering OPTIMIZED: Uses repository methods with JOIN
    ...
    public ResponseEntity<PerfectPanelResponse<Page<OrderResponse>>> getOrders(
            @Parameter(description = "Filter by order status")
                    ...
                    @Pattern(
                            regexp =
                                    "PENDING|IN_PROGRESS|PROCESSING|ACTIVE|PARTIAL|COMPLETED|CANCELLED|PAUSED|HOLDING|REFILL",
            ...
            @Parameter(description = "Sort field")
                    ...
                    @Pattern(
                            regexp = "id|createdAt|updatedAt|status|quantity",
    ...
    /**
     * Get specific order by ID OPTIMIZED: Uses repository method with JOIN FETCH to prevent N+1
     * queries
     */
    @GetMapping("/{id}")
    @Operation(
    ...
    public ResponseEntity<PerfectPanelResponse<Void>> cancelOrder(
        ...
        PerfectPanelResponse<Void> response =
                PerfectPanelResponse.<Void>builder()
                        .success(true)
    ...
    public ResponseEntity<PerfectPanelResponse<OrderStatistics>> getOrderStatistics(
            @Parameter(description = "Days to look back")
                    ...
                    @jakarta.validation.constraints.Max(
                            value = 365,
                            message = "Cannot look back more than 365 days")
                    int days) {

        OrderStatistics stats = orderService.getOrderStatistics(days);

        PerfectPanelResponse<OrderStatistics> response =
                PerfectPanelResponse.<OrderStatistics>builder()
                        .data(stats)
                        .success(true)
    ...
    public ResponseEntity<PerfectPanelResponse<BulkOperationResult>> bulkOperation(
            @Valid @RequestBody BulkOrderRequest request, Principal principal) {

        BulkOperationResult result = orderService.performBulkOperation(request);

        PerfectPanelResponse<BulkOperationResult> response =
                PerfectPanelResponse.<BulkOperationResult>builder()
                        .data(result)
                        .success(true)
    ...
    public ResponseEntity<PerfectPanelResponse<HealthStatus>> healthCheck() {
        ...
        PerfectPanelResponse<HealthStatus> response =
                PerfectPanelResponse.<HealthStatus>builder()
                        .data(health)
