...
/**
 * Comprehensive security tests for API key generation, hashing, and collision resistance
 * Tests various attack scenarios and validates cryptographic security
 */
@SpringBootTest
@ActiveProfiles("test")
class ApiKeySecurityTest {
    ...
    void testConstantTimeComparison() throws NoSuchAlgorithmException {
        ...
        assertTrue(apiKeyGenerator.verifyApiKey(apiKey, correctHash, salt), 
            "Correct API key should verify successfully");
        
        // Test incorrect API key
        assertFalse(apiKeyGenerator.verifyApiKey("wrong-key", correctHash, salt), 
            "Incorrect API key should fail verification");
        
        // Test incorrect hash
        String wrongHash = apiKeyGenerator.hashApiKey("different-key", salt);
        assertFalse(apiKeyGenerator.verifyApiKey(apiKey, wrongHash, salt), 
            "Incorrect hash should fail verification");
        
        // Test incorrect salt
        String wrongSalt = apiKeyGenerator.generateSalt();
        assertFalse(apiKeyGenerator.verifyApiKey(apiKey, correctHash, wrongSalt), 
    ...
    void testHashCollisionResistance() throws NoSuchAlgorithmException {
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            assertFalse(generatedHashes.contains(hash), 
    ...
    void testAvalancheEffect() throws NoSuchAlgorithmException {
        ...
        String hash3 = apiKeyGenerator.hashApiKey("A" + baseKey.substring(1), salt); // Change first char
    ...
    void testHashPerformance() throws NoSuchAlgorithmException {
        ...
        assertTrue(durationMs < 1000, "Hash performance is too slow: " + durationMs + "ms for 1000 operations");
        
        System.out.println("Hash performance: " + durationMs + "ms for 1000 operations (" + 
            (durationMs / 1000.0) + "ms per hash)");
    }
    ...
    void testGlobalSaltSecurity() throws NoSuchAlgorithmException {
        ...
        assertNotEquals(lookupHash, verificationHash, 
            "Global salt hash should differ from user salt hash");
