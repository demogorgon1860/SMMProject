 ...
 *
 * <p>Tests various error scenarios and recovery mechanisms: 1. Retryable exceptions with
 * exponential backoff 2. Non-retryable exceptions that go directly to DLQ 3. Dead letter queue
 * message processing 4. Error handler configuration validation 5. Metrics and monitoring
 * verification
 */
@Slf4j
@SpringBootTest
...
class KafkaErrorHandlingIntegrationTest {
    ...
    void setUp() {
        ...
        dlqConsumer =
                new DefaultKafkaConsumerFactory<String, Object>(dlqConsumerProps).createConsumer();
    ...
    void testDefaultErrorHandlerCreation() {
        ...
        assertTrue(
                errorHandler instanceof org.springframework.kafka.listener.DefaultErrorHandler,
    ...
    void testRetryableExceptionHandling() throws Exception {
        ...
        log.info(
                "Retryable exception handling test completed with {} attempts", attemptCount.get());
    }
    ...
    void testNonRetryableExceptionHandling() throws Exception {
                ...
                () -> {
                    ...
                    for (ConsumerRecord<String, Object> record : records) {
                        ...
                        throw new IllegalArgumentException(
                                "Invalid message format - non-retryable");
                    }
                });
        ...
        CompletableFuture.runAsync(
                () -> {
                    ConsumerRecords<String, Object> dlqRecords =
                            dlqConsumer.poll(Duration.ofMillis(5000));
        ...
        assertEquals(
                1,
                attemptCount.get(),
                "Should have attempted only once for non-retryable exception");
    ...
    void testDeadLetterQueueProcessing() throws Exception {
        ...
        Map<String, Object> dlqMessage =
                Map.of(
                        ...
                        "test-key",
                        "originalValue",
                        Map.of("orderId", 12345L, "videoUrl", "https://test.com"),
                        "errorMessage",
                        "Processing timeout",
                        "errorClass",
                        "TimeoutException",
        ...
        CompletableFuture.runAsync(
                () -> {
                    ConsumerRecords<String, Object> records =
                            dlqConsumer.poll(Duration.ofMillis(3000));
                    for (ConsumerRecord<String, Object> record : records) {
                        ...
                        assertTrue(
                                message.containsKey("originalTopic"),
                                "DLQ message should contain original topic");
                        assertTrue(
                                message.containsKey("errorMessage"),
                                "DLQ message should contain error message");
                        assertTrue(
                                message.containsKey("timestamp"),
                                "DLQ message should contain timestamp");
    ...
    void testHighPriorityErrorHandling() throws Exception {
        ...
        Map<String, Object> highPriorityMessage =
                Map.of(
                        "orderId",
                        99999L,
                        "priority",
                        "HIGH",
                        "processingType",
                        "URGENT_VIDEO_PROCESSING",
                        "timestamp",
                        LocalDateTime.now().toString());

        testProducer.send(
                new ProducerRecord<>(
                        "test.high.priority", "high-priority-key", highPriorityMessage));
        testProducer.flush();
    ...
    void testErrorMetricsCollection() {
        ...
        assertTrue(
                metrics.containsKey("nonRetryableErrors"),
                "Should contain non-retryable errors count");
    ...
    void testDlqTopicResolution() {
        ...
        assertEquals(
                originalTopics.length,
                expectedDlqTopics.length,
    ...
    void testConcurrentErrorHandling() throws Exception {
        ...
        for (int i = 0; i < messageCount; i++) {
            ...
            testProducer.send(
                    new ProducerRecord<>("test.video.processing", "concurrent-key-" + i, message));
        }
        testProducer.flush();
        ...
        log.info(
                "Concurrent error handling test completed - processed {} messages",
                processedCount.get());
    }
}
