 ...
 * 
 * Provides centralized, thread-safe order state management with:
 * 1. Atomic state transitions with validation
 * 2. State transition logging and audit trail
 * 3. Async operation status tracking
 * 4. Consistency guarantees across concurrent operations
 * 5. Rollback capabilities for failed transitions
 */
@Slf4j
@Service
...
public class OrderStateManagementService {
     ...
     * Defines allowed transitions to prevent invalid state changes
     */
    private static final Map<OrderStatus, EnumSet<OrderStatus>> VALID_TRANSITIONS = Map.of(
        OrderStatus.PENDING, EnumSet.of(OrderStatus.PROCESSING, OrderStatus.CANCELLED, OrderStatus.HOLDING),
        OrderStatus.PROCESSING, EnumSet.of(OrderStatus.ACTIVE, OrderStatus.HOLDING, OrderStatus.CANCELLED),
        OrderStatus.ACTIVE, EnumSet.of(OrderStatus.COMPLETED, OrderStatus.HOLDING, OrderStatus.CANCELLED),
        OrderStatus.HOLDING, EnumSet.of(OrderStatus.PROCESSING, OrderStatus.CANCELLED),
        OrderStatus.COMPLETED, EnumSet.of(OrderStatus.HOLDING), // Only allow hold for investigation
        OrderStatus.CANCELLED, EnumSet.noneOf(OrderStatus.class) // Terminal state
    );

    /**
     * IMMEDIATE VALIDATION AND STATUS UPDATE
     * Updates order status immediately after basic validation
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public OrderValidationResult validateAndUpdateOrderForProcessing(Long orderId, String youtubeVideoId) {
        try {
            ...
            if (!order.getStatus().equals(OrderStatus.PENDING)) {
                ...
                return OrderValidationResult.failed(orderId, "Order not in PENDING status: " + order.getStatus());
            }
            ...
            logStateTransition(orderId, previousStatus, OrderStatus.PROCESSING, "Order validated and queued for processing");
     ...
     * Sets detailed processing status to indicate specific async work in progress
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void updateProcessingStatus(Long orderId, ProcessingPhase phase, String details) {
        try {
            ...
            if (!order.getStatus().equals(OrderStatus.PROCESSING)) {
                log.warn("Attempting to update processing status for order {} not in PROCESSING state: {}", 
            ...
            log.info("Updated processing status for order {}: phase={}, details={}", orderId, phase, details);

        } catch (Exception e) {
            log.error("Failed to update processing status for order {}: {}", orderId, e.getMessage(), e);
        }
    }

    /**
     * TRANSITION TO ACTIVE STATE
     * Completes the processing phase and activates the order
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public StateTransitionResult transitionToActive(Long orderId, int startCount) {
        try {
            log.info("Transitioning order {} to ACTIVE state with start count: {}", orderId, startCount);
            ...
            // Log state transition
            logStateTransition(orderId, previousStatus, OrderStatus.ACTIVE, 
            ...
            return StateTransitionResult.failed(orderId, OrderStatus.PROCESSING, OrderStatus.ACTIVE, e.getMessage());
        }
    }

    /**
     * TRANSITION TO COMPLETED STATE
     * Marks order as completed when target is reached
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public StateTransitionResult transitionToCompleted(Long orderId, int finalCount) {
        try {
            log.info("Transitioning order {} to COMPLETED state with final count: {}", orderId, finalCount);

            Order order = orderRepository.findById(orderId)
                    .orElseThrow(() -> new VideoProcessingException("Order not found: " + orderId));

            // Validate transition
            StateTransitionResult validationResult = validateTransition(order, OrderStatus.COMPLETED);
            if (!validationResult.isSuccess()) {
                return validationResult;
            }
            ...
            // Log state transition
            logStateTransition(orderId, previousStatus, OrderStatus.COMPLETED, 
            ...
            return StateTransitionResult.failed(orderId, OrderStatus.ACTIVE, OrderStatus.COMPLETED, e.getMessage());
        }
    }

    /**
     * TRANSITION TO HOLDING STATE
     * Moves order to holding state for error investigation
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public StateTransitionResult transitionToHolding(Long orderId, String reason) {
        ...
        try {
            ...
            return StateTransitionResult.failed(orderId, currentStatus, OrderStatus.HOLDING, e.getMessage());
        }
    }

    /**
     * UPDATE ORDER PROGRESS
     * Updates remaining count and checks for completion
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public ProgressUpdateResult updateOrderProgress(Long orderId, int currentViews) {
        try {
            ...
            if (!order.getStatus().equals(OrderStatus.ACTIVE)) {
                log.debug("Skipping progress update for order {} not in ACTIVE state: {}", orderId, order.getStatus());
            ...
            if (completed) {
                ...
                logStateTransition(orderId, OrderStatus.ACTIVE, OrderStatus.COMPLETED, 
                        String.format("Auto-completed: %d views gained, target reached", viewsGained));
            }

            orderRepository.save(order);

            log.debug("Updated progress for order {}: views gained={}, remains={}, completed={}", 
                    orderId, viewsGained, remains, completed);
     ...
     * Returns current processing state information
     */
    public Optional<ProcessingStateInfo> getProcessingState(Long orderId) {
        ...
        return Optional.of(ProcessingStateInfo.builder()
                .orderId(orderId)
     ...
     * Removes processing states that have been active too long (likely stuck)
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void cleanupStaleProcessingStates(int maxProcessingMinutes) {
        ...
        activeProcessingStates.entrySet().removeIf(entry -> {
            ProcessingState state = entry.getValue();
            if (state.getStartTime().isBefore(cutoffTime)) {
                Long orderId = entry.getKey();
                log.warn("Cleaning up stale processing state for order {}: started at {}", 
                        orderId, state.getStartTime());
                
                // Optionally transition stale orders to holding
                try {
                    transitionToHolding(orderId, "Processing timeout - cleaned up stale state");
                } catch (Exception e) {
                    log.error("Failed to transition stale order {} to holding: {}", orderId, e.getMessage());
                }
     ...
     * Checks if transition is allowed and safe
     */
    private StateTransitionResult validateTransition(Order order, OrderStatus targetStatus) {
        ...
        if (allowedTransitions == null || !allowedTransitions.contains(targetStatus)) {
            String message = String.format("Invalid state transition: %s → %s", currentStatus, targetStatus);
            log.warn("Order {}: {}", order.getId(), message);
            return StateTransitionResult.failed(order.getId(), currentStatus, targetStatus, message);
        }
    ...
    /**
     * LOG STATE TRANSITION
     * Creates audit trail for state changes
     */
    private void logStateTransition(Long orderId, OrderStatus fromStatus, OrderStatus toStatus, String reason) {
        log.info("ORDER STATE TRANSITION: orderId={}, {} → {}, reason: {}", 
                orderId, fromStatus, toStatus, reason);
        
        // In a production system, this would also write to an audit table
        // auditService.logStateTransition(orderId, fromStatus, toStatus, reason, LocalDateTime.now());
    }
    ...
    private static class ProcessingState {
        ...
        public void setCurrentPhase(ProcessingPhase currentPhase) { this.currentPhase = currentPhase; }
        public String getPhaseDetails() { return phaseDetails; }
        public void setPhaseDetails(String phaseDetails) { this.phaseDetails = phaseDetails; }
...
class StateTransitionResult {
    ...
    public static StateTransitionResult success(Long orderId, OrderStatus fromStatus, OrderStatus toStatus) {
    ...
    public static StateTransitionResult failed(Long orderId, OrderStatus fromStatus, OrderStatus toStatus, String errorMessage) {
        return StateTransitionResult.builder()
                .orderId(orderId)
                .success(false)
                .fromStatus(fromStatus)
...
class ProgressUpdateResult {
    ...
    public static ProgressUpdateResult success(Long orderId, int viewsGained, int remains, boolean completed) {
