...
/**
 * Specific tests for repository query performance and optimization
 * Focuses on testing the optimized repository methods we've implemented
 */
@Slf4j
public class RepositoryQueryPerformanceTest extends QueryPerformanceTestBase {
    ...
    public void testOptimizedOrderRepositoryFindWithDetails() {
        ...
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                Optional<Order> order = orderRepository.findByIdWithAllDetails(testOrder.getId());
    ...
    public void testActiveOrdersForProcessing() {
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                for (Order order : activeOrders.subList(0, Math.min(3, activeOrders.size()))) {
                    String username = order.getUser().getUsername();
                    String serviceName = order.getService().getName();
    ...
    public void testUserStatisticsAggregation() {
        ...
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                Long orderCount = orderRepository.countByUser_Username(testUser.getUsername());
                var totalSpent = orderRepository.sumChargeByUser_Username(testUser.getUsername());
                Long pendingCount = orderRepository.countByUser_UsernameAndStatus(
                    testUser.getUsername(), OrderStatus.PENDING
                );
                var recentSpent = orderRepository.sumChargeByUser_UsernameAndCreatedAtAfter(
                    testUser.getUsername(), LocalDateTime.now().minusDays(30)
                );
    ...
    public void testErrorRecoveryOrders() {
        ...
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                Page<Order> dlqOrders = orderRepository.findDeadLetterQueueOrders(pageable);
    ...
    public void testComplexSearchScenarios() {
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                List<Order> recentOrders = orderRepository.findOrdersCreatedAfter(weekAgo);
                ...
                if (!errorTypes.isEmpty()) {
                    ...
                    Page<Order> errorOrders = orderRepository.findOrdersByErrorType(errorType, pageable);
    ...
    public void testRepositoryConcurrentAccessSimulation() {
        QueryPerformanceResult result = measureQueryPerformance(
            ...
            () -> {
                ...
                Page<Order> user1Orders = orderRepository.findOrdersWithDetailsByUserId(
                    user1.getId(), pageable1
                );
                
                // Operation 2: User 2 order listing
                Pageable pageable2 = PageRequest.of(0, 5);
                Page<Order> user2Orders = orderRepository.findOrdersWithDetailsByUserId(
