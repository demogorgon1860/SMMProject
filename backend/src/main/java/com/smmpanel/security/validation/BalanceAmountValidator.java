 ...
 *
 * <p>SECURITY FEATURES: - Currency-specific amount limits - Fraud detection for suspicious amounts
 * - Precision validation for cryptocurrencies - Daily limit enforcement preparation - Anti-money
 * laundering (AML) patterns
 */
@Slf4j
@Component
public class BalanceAmountValidator implements ConstraintValidator<BalanceAmount, BigDecimal> {
    ...
    private static final Map<BalanceAmount.OperationType, BigDecimal> DEFAULT_MIN_LIMITS =
            Map.of(
    ...
    private static final Map<BalanceAmount.OperationType, BigDecimal> DEFAULT_MAX_LIMITS =
            Map.of(
    ...
    private static final Map<String, CurrencyConfig> CURRENCY_CONFIGS =
            Map.of(
                    "USD",
                            new CurrencyConfig(
                                    new BigDecimal("5.00"), new BigDecimal("50000.00"), 2),
                    "EUR",
                            new CurrencyConfig(
                                    new BigDecimal("5.00"), new BigDecimal("45000.00"), 2),
                    ...
                    "USDT",
                            new CurrencyConfig(
                                    new BigDecimal("5.00"), new BigDecimal("50000.00"), 6),
                    "USDC",
                            new CurrencyConfig(
                                    new BigDecimal("5.00"), new BigDecimal("50000.00"), 6),
    ...
    private boolean validateDecimalPrecision(
            BigDecimal amount, String currency, ConstraintValidatorContext context) {
        CurrencyConfig config =
                CURRENCY_CONFIGS.getOrDefault(currency, CURRENCY_CONFIGS.get("USD"));

        if (amount.scale() > config.decimalPlaces) {
            addConstraintViolation(
                    context,
                    String.format(
                            "Amount precision exceeds %d decimal places for %s",
    ...
    private boolean validateAmountRange(
            BigDecimal amount, String currency, ConstraintValidatorContext context) {
    ...
    private boolean validateFraudDetection(BigDecimal amount, ConstraintValidatorContext context) {
        ...
        if (usdAmount.compareTo(HIGH_RISK_AMOUNT_THRESHOLD) > 0) {
            ...
            addConstraintViolation(
                    context, "Amount exceeds high-risk threshold. Enhanced verification required.");
            return false;
        }
        ...
        if (isSuspiciousAmount(amount)) {
            ...
            addConstraintViolation(
                    context, "Amount appears suspicious. Please verify transaction details.");
            return false;
        }
    ...
    private BigDecimal convertToUsdEquivalent(BigDecimal amount, String currency) {
        ...
        switch (currency.toUpperCase()) {
            case "BTC":
                return amount.multiply(new BigDecimal("45000")); // Approx BTC price
            case "ETH":
                return amount.multiply(new BigDecimal("2500")); // Approx ETH price
            case "LTC":
                return amount.multiply(new BigDecimal("100")); // Approx LTC price
            case "USDT":
            case "USDC":
                return amount; // 1:1 with USD
            case "EUR":
                return amount.multiply(new BigDecimal("1.1")); // Approx EUR/USD
            default:
