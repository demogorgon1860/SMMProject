...
public class TransactionMonitoringController {
    ...
    public ResponseEntity<Map<String, Object>> getTransactionMetrics() {
        ...
        for (Map.Entry<String, TransactionMonitoringAspect.TransactionMetrics> entry : metrics.entrySet()) {
        ...
        long totalAttempts = metrics.values().stream().mapToLong(TransactionMonitoringAspect.TransactionMetrics::getAttempts).sum();
        long totalSuccesses = metrics.values().stream().mapToLong(TransactionMonitoringAspect.TransactionMetrics::getSuccesses).sum();
        long totalFailures = metrics.values().stream().mapToLong(TransactionMonitoringAspect.TransactionMetrics::getFailures).sum();
        ...
        overallStats.put("overallSuccessRate", totalAttempts > 0 ? (double) totalSuccesses / totalAttempts * 100 : 0.0);
    ...
    @Operation(summary = "Get metrics for specific method pattern",
               description = "Returns transaction metrics for methods matching the given pattern")
    public ResponseEntity<Map<String, Object>> getMethodMetrics(@PathVariable String methodPattern) {
        ...
        for (Map.Entry<String, TransactionMonitoringAspect.TransactionMetrics> entry : allMetrics.entrySet()) {
    ...
    @Operation(summary = "Reset transaction metrics",
    ...
    @GetMapping("/health")
    @Operation(summary = "Transaction system health check",
    ...
    public ResponseEntity<Map<String, Object>> getTransactionHealth() {
        ...
        double overallFailureRate = totalAttempts > 0 ? (double) totalFailures / totalAttempts * 100 : 0.0;
        
        // Check for concerning patterns
        boolean hasHighFailureRate = overallFailureRate > 10.0; // More than 10% failure rate
        boolean hasSlowTransactions = metrics.values().stream()
            .anyMatch(m -> m.getMaxDuration() > 10000); // Any transaction taking more than 10 seconds
        ...
        if (hasHighFailureRate) {
            recommendations.put("highFailureRate", "Review failing transactions and consider optimizing retry logic");
        }
        if (hasSlowTransactions) {
            recommendations.put("slowTransactions", "Investigate slow transactions and consider optimizing database queries or isolation levels");
        }
        if (overallFailureRate > 5.0) {
            recommendations.put("moderateFailures", "Monitor transaction patterns and consider adjusting timeout settings");
        }
