...
public class FraudDetectionService {
    ...
    private static final int DUPLICATE_WINDOW_MINUTES = 10; // 10 minutes window for duplicate detection
    private static final int SUSPICIOUS_ORDERS_PER_HOUR = 20; // Threshold for suspicious orders
    private static final int REPEATED_QUANTITY_THRESHOLD = 10; // Threshold for repeated quantity pattern
    ...
    private boolean isDuplicateOrder(Long userId, CreateOrderRequest request) {
        ...
        long duplicateCount = orderRepository.countByUserIdAndLinkAndCreatedAtAfter(
    ...
    private boolean isSuspiciousPattern(Long userId, CreateOrderRequest request) {
        ...
        if (!recentOrders.isEmpty()) {
            long sameQuantityCount = recentOrders.stream()
