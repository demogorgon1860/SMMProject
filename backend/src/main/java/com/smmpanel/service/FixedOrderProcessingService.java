...
public class FixedOrderProcessingService {
    ...
    private void assignOfferDirectly(Order order) {
        try {
            String offerName =
                    "SMM_ORIGINAL_Order_" + order.getId() + "_" + System.currentTimeMillis();

            // Отправляем событие для назначения оффера с оригинальной ссылкой
            offerEventProducer.sendOfferAssignmentEvent(
                    order.getId(), offerName, order.getLink(), "DIRECT_ASSIGNMENT");
            ...
            log.error(
                    "Failed to assign offer directly for order {}: {}",
                    order.getId(),
                    e.getMessage(),
                    e);
            throw e;
    ...
    /** Определение необходимости создания клипа (упрощенная логика - можно расширить) */
