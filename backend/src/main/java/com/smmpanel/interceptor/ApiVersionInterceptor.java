 ...
 * Validates API version headers and handles version compatibility
 */
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (apiVersion == null || apiVersion.isEmpty()) {
            response.sendError(HttpStatus.BAD_REQUEST.value(), 
                "Missing required header: " + VERSION_HEADER);
            return false;
        }
        ...
        if (!isVersionSupported(apiVersion)) {
            response.sendError(HttpStatus.BAD_REQUEST.value(),
                "Unsupported API version. Minimum supported version: " + MINIMUM_SUPPORTED_VERSION);
            return false;
        }
