...
/**
 * Integration tests to verify that controller optimizations work end-to-end and prevent N+1 query
 * issues in real HTTP requests
 */
...
public class ControllerOptimizationIntegrationTest {
    ...
    public void testOrderListEndpointQueryCount() throws Exception {
        ...
        assertTrue(
                queryCount <= 3,
                "Order listing should use at most 3 queries (user lookup + paginated orders +"
                        + " count), got: "
                        + queryCount);
    }
    ...
    public void testSingleOrderEndpointQueryCount() throws Exception {
        ...
        assertTrue(
                queryCount <= 2,
                "Single order fetch should use at most 2 queries (user lookup + order with"
                        + " details), got: "
                        + queryCount);
    }
    ...
    public void testOrderStatisticsEndpointQueryCount() throws Exception {
        ...
        // Statistics endpoints may need more queries for aggregations, but should still be
        // reasonable
        assertTrue(
                queryCount <= 10,
    ...
    public void testPerfectPanelServicesEndpointCaching() throws Exception {
        ...
        assertTrue(
                secondCallQueryCount <= firstCallQueryCount,
