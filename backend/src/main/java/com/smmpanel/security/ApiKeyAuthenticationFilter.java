 ...
 * 6. Added rate limiting for brute force protection
 */
@Slf4j
@Component
...
public class ApiKeyAuthenticationFilter extends OncePerRequestFilter {
            ...
            if (userOpt.isPresent()) {
                ...
                if (validateApiKeyWithRateLimit(apiKey, user, clientIdentifier)) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Authenticated user {} with API key - Timing: {}", 
                            user.getUsername(), stopWatch.prettyPrint());
                    } else {
                        log.info("Authenticated user {} - Total time: {}ms", 
                            user.getUsername(), stopWatch.getTotalTimeMillis());
                    }
                    ...
                    stopWatch.stop();
                    log.warn("API key verification failed for user {} - Timing: {}", 
                        user.getUsername(), stopWatch.prettyPrint());
                ...
                log.warn("Invalid API key provided - Hash lookup time: {}ms", 
                    stopWatch.getLastTaskTimeMillis());
                throw new SecurityException("Invalid API key");
            }
        } catch (DataAccessException e) {
            log.error("Database connection error during API key authentication: {}", e.getMessage());
    ...
    private boolean validateApiKeyWithRateLimit(String apiKey, User user, String clientIdentifier) {
        ...
        try {
            return apiKeyService.verifyApiKeyOnly(apiKey, user.getApiKeyHash(), 
                user.getApiKeySalt(), clientIdentifier);
        } catch (Exception e) {
