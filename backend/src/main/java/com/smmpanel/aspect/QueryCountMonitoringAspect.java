...
/**
 * Aspect to monitor query count for controller endpoints Helps identify N+1 query problems in
 * production
 */
@Aspect
@Component
...
public class QueryCountMonitoringAspect {
    ...
    public Object monitorControllerQueries(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (queryCount >= queryErrorThreshold) {
                log.error(
                        "HIGH QUERY COUNT DETECTED - Method: {} | Queries: {} | Entities: {} |"
                                + " Collections: {} | Time: {}ms",
                        methodName,
                        queryCount,
                        entityLoadCount,
                ...
                log.warn(
                        "Elevated query count - Method: {} | Queries: {} | Entities: {} |"
                                + " Collections: {} | Time: {}ms",
                        methodName,
                        queryCount,
                        entityLoadCount,
                ...
                log.debug(
                        "Query stats - Method: {} | Queries: {} | Entities: {} | Collections: {} |"
                                + " Time: {}ms",
                        methodName,
                        queryCount,
                        entityLoadCount,
            ...
            log.error(
                    "Controller method failed - Method: {} | Queries before failure: {} | Time:"
                            + " {}ms | Error: {}",
                    methodName,
                    queryCount,
                    executionTime,
    ...
    private Object monitorServiceMethodQueries(ProceedingJoinPoint joinPoint, String serviceName)
            throws Throwable {
        if (!queryCountMonitoringEnabled) {
            return joinPoint.proceed();
        }
        ...
        try {
            ...
            if (queryCount > 2) { // Service methods should typically use 1-2 queries max
                log.warn(
                        "Service method used {} queries - Method: {} | Time: {}ms",
                        queryCount,
                        methodName,
                        executionTime);
            } else if (log.isDebugEnabled()) {
                log.debug(
                        "Service method stats - Method: {} | Queries: {} | Time: {}ms",
                        methodName,
                        queryCount,
                        executionTime);
            }
            ...
            log.error(
                    "Service method failed - Method: {} | Queries: {} | Time: {}ms | Error: {}",
                    methodName,
                    queryCount,
                    executionTime,
                    throwable.getMessage());
