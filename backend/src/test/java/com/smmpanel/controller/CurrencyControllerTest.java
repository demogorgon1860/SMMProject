...
class CurrencyControllerTest {
    ...
    @Mock private UserCurrencyPreferenceService userCurrencyPreferenceService;
    ...
    private final CurrencyService.CurrencyInfo usdInfo =
            new CurrencyService.CurrencyInfo("$", 2, "US Dollar", false);
    private final CurrencyService.CurrencyInfo eurInfo =
            new CurrencyService.CurrencyInfo("€", 2, "Euro", true);
    private final CurrencyService.CurrencyInfo gbpInfo =
    ...
    void getSupportedCurrencies_ShouldReturnListOfCurrencies() throws Exception {
        mockMvc.perform(get("/api/currency/supported").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.USD").exists())
                .andExpect(jsonPath("$.data.USD.symbol").value("$"))
    ...
    void getSupportedCurrenciesWithPreference_ShouldMarkUserPreferredCurrency() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/currency/supported/with-preference")
                                .with(
                                        request -> {
                                            request.setAttribute(
                                                    CurrentUser.REQUEST_ATTRIBUTE, testUser);
                                            return request;
                                        })
    ...
    void convertCurrency_ValidRequest_ReturnsConvertedAmount() throws Exception {
        CurrencyConversionRequest request =
                new CurrencyConversionRequest(new BigDecimal("100"), "USD", "EUR", true);
    ...
    void convertCurrency_ShouldConvertAmount() throws Exception {
        ...
        CurrencyConversionResultDto response =
    ...
    void getPreferredCurrency_ShouldReturnUserPreferredCurrency() throws Exception {
        when(userCurrencyPreferenceService.getUserPreferredCurrency(anyLong())).thenReturn("EUR");

        mockMvc.perform(
                        get("/api/currency/preferences/currency")
                                .with(
                                        request -> {
                                            request.setAttribute(
                                                    CurrentUser.REQUEST_ATTRIBUTE, testUser);
    ...
    void updatePreferredCurrency_ShouldUpdateUserPreference() throws Exception {
        doNothing()
                .when(userCurrencyPreferenceService)
                .updateUserPreferredCurrency(anyLong(), eq("EUR"));

        mockMvc.perform(
                        put("/api/currency/preferences/currency")
    ...
    void convertToUserCurrency_ShouldConvertAndFormatForUser() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/currency/convert-to-user-currency")
    ...
    void quickConvert_MissingParameters_ReturnsBadRequest() throws Exception {
        mockMvc.perform(get("/api/currency/convert")).andExpect(status().isBadRequest());
    }
}
