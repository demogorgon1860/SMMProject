...
@Repository
public interface OrderRepository extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order>, OptimizedOrderQueries {
    
    // OPTIMIZED QUERIES WITH JOIN FETCH TO PREVENT N+1 PROBLEMS
    
    /**
     * Find orders by user with all related entities fetched
     * PREVENTS N+1: Fetches user and service in single query
     */
    @EntityGraph(attributePaths = {"user", "service"})
    Page<Order> findByUser(User user, Pageable pageable);

    /**
     * Find orders by user and status with related entities
     * PREVENTS N+1: Fetches user and service in single query
     */
    ...
    /**
     * Find single order by ID and user with related entities
     * PREVENTS N+1: Fetches user and service in single query
     */
    @Query("SELECT o FROM Order o JOIN FETCH o.user JOIN FETCH o.service " +
    ...
    /**
     * Find orders by status with related entities for admin views
     * PREVENTS N+1: Fetches user and service for each order
     */
    @Query("SELECT o FROM Order o JOIN FETCH o.user JOIN FETCH o.service " +
    ...
    /**
     * Find orders by multiple statuses with related entities
     * PREVENTS N+1: Fetches user and service for admin dashboards
     */
    @Query("SELECT o FROM Order o JOIN FETCH o.user JOIN FETCH o.service " +
    ...
    /**
     * Find orders created after date with all details
     * PREVENTS N+1: Fetches user and service for reporting
     */
    @Query("SELECT o FROM Order o JOIN FETCH o.user JOIN FETCH o.service " +
    ...
    /**
     * SPECIALIZED QUERY: Find orders with full details by user ID
     * PREVENTS N+1: Single query with all related entities
     * INCLUDES: user, service, videoProcessing, binomCampaigns
     */
    ...
    /**
     * SPECIALIZED QUERY: Find orders with details by user ID with pagination
     * PREVENTS N+1: Single query with all related entities
     */
    ...
    /**
     * OPTIMIZED: Single order by ID with all details
     * PREVENTS N+1: Fetches all related entities in one query
     */
    ...
    /**
     * OPTIMIZED: Find active orders for processing with details
     * PREVENTS N+1: Used by processing services
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
    ...
    /**
     * OPTIMIZED: Find pending orders for queue processing
     * PREVENTS N+1: Used by automation services
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
    ...
    /**
     * Find orders ready for retry with details
     * PREVENTS N+1: Includes user and service for processing context
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
    ...
    /**
     * Find orders in dead letter queue with details
     * PREVENTS N+1: Includes user and service for operator review
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
     ...
     */
    @Query("SELECT COUNT(o) FROM Order o WHERE o.errorMessage IS NOT NULL AND o.updatedAt >= :since")
    long countFailedOrdersSince(@Param("since") LocalDateTime since);

    /**
     * Count orders in dead letter queue
     */
    @Query("SELECT COUNT(o) FROM Order o WHERE (o.isManuallyFailed = true OR o.retryCount >= o.maxRetries) " +
     ...
     */
    @Query("SELECT COUNT(o) FROM Order o WHERE o.nextRetryAt IS NOT NULL AND o.nextRetryAt > :now " +
           "AND o.isManuallyFailed = false AND o.retryCount < o.maxRetries")
     ...
     */
    @Query("SELECT o.lastErrorType as errorType, COUNT(o) as count " +
    ...
    /**
     * Find orders with specific error type with details
     * PREVENTS N+1: Includes user and service for error analysis
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
    ...
    /**
     * Find orders that failed in specific phase with details
     * PREVENTS N+1: Includes user and service for phase analysis
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
    ...
    /**
     * Find orders with high retry count with details
     * PREVENTS N+1: Includes user and service for retry analysis
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
           "WHERE o.retryCount >= :minRetries ORDER BY o.retryCount DESC, o.updatedAt DESC")
    Page<Order> findOrdersWithHighRetryCount(@Param("minRetries") int minRetries, Pageable pageable);

    /**
     * Get orders for manual operator review with details
     * PREVENTS N+1: Includes user and service for operator dashboard
     */
    @Query("SELECT o FROM Order o " +
           "JOIN FETCH o.user u " +
           "JOIN FETCH o.service s " +
