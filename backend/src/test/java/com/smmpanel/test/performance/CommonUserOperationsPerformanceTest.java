...
/**
 * Performance tests for common user operations in the SMM Panel
 * Tests real-world scenarios with actual query counting and performance assertions
 */
@Slf4j
public class CommonUserOperationsPerformanceTest extends QueryPerformanceTestBase {
    ...
    public void setupTestData() {
        ...
        log.info("Test data setup completed: {} users, {} services, {} orders", 
            testUsers.size(), testServices.size(), testOrders.size());
    }

    @Test
    @DisplayName("User Registration and Profile Creation Performance")
    public void testUserRegistrationPerformance() {
        QueryPerformanceResult result = measureQueryPerformance("User Registration", () -> {
        ...
        assert result.getExecutionTimeMs() < 100 : "User registration should complete in under 100ms";
    }
    ...
    public void testOrderCreationPerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("Order Creation", () -> {
    ...
    public void testOrderHistoryListingPerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("Order History Listing", () -> {
        ...
        assert result.getExecutionTimeMs() < 300 : "Order history listing should complete in under 300ms";
    }
    ...
    public void testSingleOrderFetchPerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("Single Order Fetch", () -> {
            // Test fetching single order with all details
            Order order = orderRepository.findByIdWithAllDetails(testOrder.getId()).orElse(null);
        ...
        assert result.getExecutionTimeMs() < 100 : "Single order fetch should complete in under 100ms";
    }
    ...
    public void testBalanceCheckPerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("Balance Check", () -> {
            BigDecimal userBalance = balanceService.getUserBalance(testUser.getId());
            boolean hasSufficientBalance = balanceService.hasSufficientBalance(testUser.getId(), checkAmount);
            
            assert userBalance != null;
            assert hasSufficientBalance || !hasSufficientBalance; // Just check it returns a boolean
        });
    ...
    public void testUserProfileWithStatsPerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("User Profile with Stats", () -> {
            ...
            Long orderCount = orderRepository.countByUser_Username(user.getUsername());
            BigDecimal totalSpent = orderRepository.sumChargeByUser_Username(user.getUsername());
        ...
        assert result.getExecutionTimeMs() < 150 : "User profile loading should complete in under 150ms";
    }
    ...
    public void testOrderStatusUpdatePerformance() {
        ...
        QueryPerformanceResult result = measureQueryPerformance("Order Status Update", () -> {
        ...
        assert result.getExecutionTimeMs() < 100 : "Order status update should complete in under 100ms";
    }

    @Test
    @DisplayName("Search Orders Performance")
    public void testSearchOrdersPerformance() {
        QueryPerformanceResult result = measureQueryPerformance("Search Orders", () -> {
            // Test searching orders by different criteria
            Pageable pageable = PageRequest.of(0, 10);
            
            // Search by status
            List<Order> activeOrders = orderRepository.findByStatusIn(
                List.of(com.smmpanel.entity.OrderStatus.ACTIVE, com.smmpanel.entity.OrderStatus.PROCESSING)
            );
            
            // Search by user
            Page<Order> userOrders = orderRepository.findByUser(testUsers.get(0), pageable);
            
            assert !activeOrders.isEmpty() || activeOrders.isEmpty(); // Just check it executes
    ...
    public void testBulkOrderStatusCheckPerformance() {
        List<Long> orderIds = testOrders.stream()
                .limit(5)
                .map(Order::getId)
                .toList();
        
        QueryPerformanceResult result = measureQueryPerformance("Bulk Order Status Check", () -> {
        ...
        assert result.getExecutionTimeMs() < 300 : "Bulk order status check should complete in under 300ms";
    }

    @Test
    @DisplayName("Dashboard Data Loading Performance")
    public void testDashboardDataLoadingPerformance() {
        QueryPerformanceResult result = measureQueryPerformance("Dashboard Data Loading", () -> {
            ...
            // Orders by status
            long pendingOrders = orderRepository.countByStatus(com.smmpanel.entity.OrderStatus.PENDING);
            long completedOrders = orderRepository.countByStatus(com.smmpanel.entity.OrderStatus.COMPLETED);
        ...
        assert result.getExecutionTimeMs() < 400 : "Dashboard data loading should complete in under 400ms";
    }
}