...
/**
 * Performance tests for UserRepository query optimizations Tests the API key authentication query
...
class UserRepositoryPerformanceTest {
    ...
    void testApiKeyQueryPerformance() {
        ...
        assertTrue(
                optimizedTime <= deprecatedTime * 1.2,
    ...
    void testOptimizedQueryOnlyReturnsActiveUsers() {
        ...
        assertEquals(
                "test_user_1", result.get().getUsername(), "Should find the correct active user");
    }
    ...
    void testPerformanceWithManyInactiveUsers() {
        ...
        for (int i = 0; i < 500; i++) {
            User inactiveUser =
                    createTestUser("inactive_" + i, "inactive" + i + "@test.com", false);
    ...
    void testQueryParameterBinding() {
        ...
        Optional<User> notFound =
                userRepository.findByApiKeyHashAndIsActiveTrue("non_existent_key");
