 ...
 *
 * <p>Features: 1. Hibernate statistics monitoring 2. N+1 query detection 3. Cache performance
 * metrics 4. Database performance insights
 *
 * <p>Security: Admin-only access
 */
@RestController
...
public class PerformanceMonitoringController {
        ...
        try {
            ...
            if (entityName != null && !entityName.isEmpty()) {
                ...
                return ResponseEntity.ok(
                        Map.of("message", "Cache evicted for entity: " + entityName));
            } else {
    ...
    public ResponseEntity<Map<String, Object>> getNPlusOneAnalysis() {
        try {
            ...
            if (collectionLoadCount > entityLoadCount) {
                possibleNPlusOne = true;
                indicators =
                        new String[] {"High collection load count - possible lazy loading issues"};
            }
            ...
            analysis.put(
                    "queryToEntityRatio",
                    entityLoadCount > 0 ? (double) queryCount / entityLoadCount : 0.0);
    ...
    private void addPerformanceInsights(Map<String, Object> stats, Statistics statistics) {
        ...
        if (cacheHitRatio < 50.0) {
            insights.put(
                    "cacheConcern", "Low cache hit ratio - consider cache configuration review");
        }
