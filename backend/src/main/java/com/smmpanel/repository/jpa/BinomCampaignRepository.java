...
public interface BinomCampaignRepository extends JpaRepository<BinomCampaign, Long> {
    ...
    @Query(
            "SELECT bc FROM BinomCampaign bc WHERE bc.clicksDelivered < bc.clicksRequired AND"
                    + " bc.status = 'ACTIVE'")
    ...
    List<BinomCampaign> findByOrderIdAndStatus(
    ...
    @Query(
            "SELECT b FROM BinomCampaign b WHERE b.campaignId = :campaignId AND b.isFixedCampaign ="
                    + " true AND b.status = 'ACTIVE'")
    Optional<BinomCampaign> findActiveFixedCampaignByCampaignId(
            @Param("campaignId") String campaignId);

    @Query(
            "SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM BinomCampaign b WHERE"
                    + " b.campaignId = :campaignId AND b.isFixedCampaign = true AND b.status ="
                    + " 'ACTIVE'")
    boolean existsActiveFixedCampaignByCampaignId(@Param("campaignId") String campaignId);

    @Query(
            "SELECT COUNT(b) FROM BinomCampaign b WHERE b.isFixedCampaign = true AND b.status ="
                    + " 'ACTIVE'")
