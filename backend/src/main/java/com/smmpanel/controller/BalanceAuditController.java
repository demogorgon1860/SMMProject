...
public class BalanceAuditController {
    ...
    @Operation(
            summary = "Reconcile user balance",
    ...
    public ResponseEntity<BalanceReconciliation> reconcileUserBalance(
        ...
        try {
            ...
            if (!reconciliation.getIsReconciled()) {
                log.warn(
                        "Balance discrepancy found for user {}: {}",
                        userId,
    ...
    @Operation(
            summary = "Trigger daily balance verification",
    ...
    public ResponseEntity<AuditTrailIntegrityReport> verifyAuditTrailIntegrity(
        ...
        log.info(
                "Verifying audit trail integrity for user {} from {} to {}",
                userId,
                fromDate,
                toDate);
        ...
        try {
            AuditTrailIntegrityReport report =
                    balanceAuditService.verifyAuditTrailIntegrity(userId, fromDate, toDate);

            if (!report.getIsIntegrityValid()) {
                log.warn(
                        "Audit trail integrity issues found for user {}: {} hash mismatches, {}"
                                + " chain breaks",
                        userId,
    ...
    public ResponseEntity<AuditTrailReport> generateAuditTrailReport(
        ...
        log.info(
                "Generating audit trail report for user {} from {} to {}",
                userId,
                startDate,
                endDate);
        ...
        try {
            AuditTrailReport report =
                    balanceAuditService.generateAuditTrailReport(userId, startDate, endDate);

            log.info(
                    "Generated audit trail report for user {}: {} transactions, net change: {}",
                    userId,
    ...
    @GetMapping("/status")
    @Operation(
            summary = "Get audit system status",
    ...
    @Operation(
            summary = "Reconcile all user balances",
