 ...
 * 2. Hibernate statistics monitoring
 * 3. N+1 query detection in development
...
public class HibernateConfig {
    ...
    /**
     * Log Hibernate statistics periodically in development
    ...
    private void logPerformanceMetrics(Statistics statistics) {
        ...
        log.info("HIBERNATE PERFORMANCE: {} queries executed, avg time: {:.2f}ms, max time: {}ms",
                queryCount, avgQueryTime, statistics.getQueryExecutionMaxTime());
                
        // Detect potential N+1 problems
        if (queryCount > 100) {
            log.warn("HIGH QUERY COUNT DETECTED: {} queries in 30 seconds - possible N+1 problem", queryCount);
        }
        
        // Detect slow queries
        if (statistics.getQueryExecutionMaxTime() > 1000) {
            log.warn("SLOW QUERY DETECTED: Max query time {}ms - SQL: {}", 
    ...
    private void logQueryStatistics(Statistics statistics) {
        log.info("HIBERNATE QUERIES: {} executed, {} cache hits, {} cache misses",
    ...
    private void logCacheStatistics(Statistics statistics) {
        ...
        log.info("SECOND LEVEL CACHE: {} puts, {} hits, {} misses (hit ratio: {:.2f}%)",
                statistics.getSecondLevelCachePutCount(),
                statistics.getSecondLevelCacheHitCount(),
                statistics.getSecondLevelCacheMissCount(),
                calculateHitRatio(statistics.getSecondLevelCacheHitCount(), 
                                statistics.getSecondLevelCacheMissCount()));
    ...
    private void logEntityCacheStats(Statistics statistics, String entityName) {
        // Note: Entity-specific cache statistics require different API approach in newer Hibernate versions
        try {
            // Get region statistics if available
            log.debug("ENTITY CACHE [{}]: Statistics available through SessionFactory.getCache()",
                    entityName.substring(entityName.lastIndexOf('.') + 1));
        } catch (Exception e) {
            // Fallback to general cache statistics
            log.debug("ENTITY CACHE [{}]: Using general cache statistics",
                    entityName.substring(entityName.lastIndexOf('.') + 1));
        }
    }
    ...
    private void logCollectionCacheStats(Statistics statistics, String collectionName) {
        ...
        try {
            log.debug("COLLECTION CACHE [{}]: Statistics available in newer Hibernate versions", 
    ...
    public String getCacheStatisticsReport() {
        ...
        return String.format(
            ...
            statistics.getQueryExecutionCount() > 0 ? 
                (double) statistics.getQueryExecutionMaxTime() / statistics.getQueryExecutionCount() : 0.0,
            statistics.getQueryExecutionMaxTime()
        );
    }
}