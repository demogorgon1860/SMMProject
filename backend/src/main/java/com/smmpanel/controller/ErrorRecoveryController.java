 ...
 * 
 * Provides operator interface for manual error recovery:
 * 1. Manual retry functionality for failed orders
 * 2. Dead letter queue management
 * 3. Error recovery statistics and monitoring
 * 4. Bulk operations for error handling
 * 5. Error analysis and reporting tools
 */
@Slf4j
@RestController
...
public class ErrorRecoveryController {
    ...
    public ResponseEntity<ApiResponse<ManualRetryResult>> manualRetryOrder(
        ...
        try {
            log.info("Manual retry requested for order {} by operator: {}", orderId, request.getOperatorNotes());

            ManualRetryResult result = errorRecoveryService.manualRetry(
                    orderId, 
                    request.getOperatorNotes(), 
                    request.isResetRetryCount()
            );

            if (result.isSuccess()) {
                return ResponseEntity.ok(ApiResponse.success(result, "Order retry initiated successfully"));
            } else {
            ...
            return ResponseEntity.internalServerError()
                    .body(ApiResponse.error("INTERNAL_ERROR", "Manual retry failed: " + e.getMessage()));
        }
    }

    /**
     * GET FAILED ORDERS
     * Returns paginated list of failed orders for operator review
     */
    ...
    public ResponseEntity<ApiResponse<Page<Order>>> getFailedOrders(
        ...
        try {
            ...
            if (errorType != null) {
                failedOrders = deadLetterQueueService.getDeadLetterQueueOrdersByErrorType(errorType, pageable);
            ...
            return ResponseEntity.ok(ApiResponse.success(failedOrders, "Failed orders retrieved successfully"));
     ...
     * Returns orders in dead letter queue
     */
    ...
    public ResponseEntity<ApiResponse<Page<Order>>> getDeadLetterQueue(
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(dlqOrders, "Dead letter queue retrieved successfully"));

        } catch (Exception e) {
            log.error("Failed to get dead letter queue: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError()
                    .body(ApiResponse.error("INTERNAL_ERROR", "Failed to retrieve dead letter queue"));
        }
    }

    /**
     * REQUEUE FROM DEAD LETTER QUEUE
     * Moves order back to processing queue
     */
    ...
    public ResponseEntity<ApiResponse<DlqOperationResult>> requeueFromDeadLetterQueue(
        ...
        try {
            log.info("Requeue from DLQ requested for order {} by operator: {}", orderId, request.getOperatorNotes());

            DlqOperationResult result = deadLetterQueueService.requeueFromDeadLetterQueue(orderId, request.getOperatorNotes());

            if (result.isSuccess()) {
                return ResponseEntity.ok(ApiResponse.success(result, "Order requeued successfully"));
            } else {
                return ResponseEntity.badRequest()
    ...
    /**
     * PURGE FROM DEAD LETTER QUEUE
     * Permanently removes order from DLQ
     */
    ...
    public ResponseEntity<ApiResponse<DlqOperationResult>> purgeFromDeadLetterQueue(
        ...
        try {
            log.info("Purge from DLQ requested for order {} by operator: {}", orderId, request.getOperatorNotes());

            DlqOperationResult result = deadLetterQueueService.purgeDeadLetterQueueEntry(orderId, request.getOperatorNotes());
    ...
    public ResponseEntity<ApiResponse<BulkRetryResult>> bulkRetryOrders(
        ...
        try {
            log.info("Bulk retry requested for {} orders by operator", request.getOrderIds().size());

            BulkRetryResult result = processBulkRetry(request);

            return ResponseEntity.ok(ApiResponse.success(result, 
                    String.format("Bulk retry completed: %d successful, %d failed", 
                            result.getSuccessfulRetries(), result.getFailedRetries())));

        } catch (Exception e) {
            log.error("Bulk retry failed: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError()
                    .body(ApiResponse.error("INTERNAL_ERROR", "Bulk retry failed: " + e.getMessage()));
        }
    }

    /**
     * GET ERROR RECOVERY STATISTICS
     * Returns comprehensive error recovery statistics
     */
    ...
    public ResponseEntity<ApiResponse<ErrorRecoveryDashboard>> getErrorRecoveryStatistics() {
        try {
            ...
            ErrorRecoveryDashboard dashboard = ErrorRecoveryDashboard.builder()
    ...
    /**
     * GET ORDER ERROR DETAILS
     * Returns detailed error information for specific order
     */
    @GetMapping("/orders/{orderId}/error-details")
    public ResponseEntity<ApiResponse<OrderErrorDetails>> getOrderErrorDetails(@PathVariable Long orderId) {
        try {
            Order order = orderRepository.findById(orderId)
                    .orElseThrow(() -> new RuntimeException("Order not found: " + orderId));

            OrderErrorDetails errorDetails = OrderErrorDetails.builder()
                    .orderId(orderId)
            ...
            return ResponseEntity.ok(ApiResponse.success(errorDetails, "Error details retrieved successfully"));
        ...
        for (Long orderId : request.getOrderIds()) {
            try {
                ...
                if (result.isSuccess()) {
                    ...
                    failed++;
                    log.warn("Bulk retry failed for order {}: {}", orderId, result.getErrorMessage());
                }
            } catch (Exception e) {
                failed++;
...
class ManualRetryRequest {
    ...
    private boolean resetRetryCount = false;
}
...
class BulkRetryRequest {
    ...
    @jakarta.validation.constraints.Size(max = 100, message = "Maximum 100 orders per bulk operation")
