...
public class SqlInjectionValidatorImpl implements ConstraintValidator<SqlInjectionSafe, String> {
    ...
    private static final List<Pattern> SQL_INJECTION_PATTERNS =
            Arrays.asList(
                    ...
                    Pattern.compile(
                            "(?i)(\\b)*(SELECT|INSERT|UPDATE|DELETE|DROP|TRUNCATE|ALTER|EXEC|UNION|CREATE|DECLARE)(\\b)*"),
                    ...
                    Pattern.compile(
                            "(?i)\\b(OR|AND)\\s+\\d+\\s*=\\s*\\d+\\b"), // OR/AND boolean conditions
                    Pattern.compile(
                            "(?i)'\\s*OR\\s*'\\w+'\\s*=\\s*'\\w+'"), // String equality tautologies
                    Pattern.compile("(?i)\\bXP_\\w+"), // Extended stored procedures
                    Pattern.compile(
                            "(?i)WAITFOR\\s+DELAY\\s+'\\d+:\\d+:\\d+'"), // Time-based attacks
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        ...
        for (Pattern pattern : SQL_INJECTION_PATTERNS) {
            if (pattern.matcher(value).find()) {
                addConstraintViolation(
                        context,
                        "Potentially harmful content detected. Please remove special characters and"
                                + " SQL keywords.");
                return false;
            }
        }
        ...
        String normalizedValue =
                value.replaceAll("\\s+", "") // Remove whitespace
        ...
        if (containsSuspiciousPatterns(normalizedValue)) {
            addConstraintViolation(
                    context,
                    "Input contains suspicious patterns. Please review and remove any special"
                            + " characters.");
            return false;
        }
