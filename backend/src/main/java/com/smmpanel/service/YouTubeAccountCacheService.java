...
/**
 * YouTube Account Availability Cache Service 30-second TTL for real-time account availability
 * status
 */
@Slf4j
@Service
...
public class YouTubeAccountCacheService {
    ...
    /**
     * Get cached YouTube account availability with 30-second TTL Returns true if account is
     * available, false if not, null if not cached
     */
    @Cacheable(
            value = "youtube-accounts",
            key =
                    "T(com.smmpanel.config.cache.BusinessEntityKeyGenerator).youtubeAccountKey(#accountId)",
            unless = "#result == null")
    ...
    public Boolean updateAccountAvailability(String accountId, Boolean isAvailable) {
        log.debug(
                "Updating cached availability for YouTube account {}: {}", accountId, isAvailable);
        return isAvailable;
    }
