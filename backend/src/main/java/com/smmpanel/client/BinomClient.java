        ...
        return executeWithRetry(() -> {
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                if (responseBody.containsKey("error")) {
                    throw new RuntimeException("Binom API error: " + responseBody.get("error"));
                }
                
                if (responseBody.containsKey("campaign_id")) {
                    String campaignId = String.valueOf(responseBody.get("campaign_id"));
                    log.info("Successfully created Binom campaign: {} -> {}", request.getName(), campaignId);
        ...
        return executeWithRetry(() -> {
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                if (responseBody.containsKey("offer_id")) {
                    ...
                    log.info("Successfully created Binom offer: {} -> {}", request.getName(), offerId);
    ...
    public AssignOfferResponse assignOfferToCampaign(String campaignId, String offerId) {
        ...
        return executeWithRetry(() -> {
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                log.info("Successfully assigned offer {} to campaign {}", offerId, campaignId);
    ...
    public CheckOfferResponse checkOfferExists(String offerName) {
        ...
        return executeWithRetry(() -> {
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                if (responseBody.containsKey("offers") && responseBody.get("offers") instanceof Map) {
                    Map<String, Object> offers = (Map<String, Object>) responseBody.get("offers");
                    if (!offers.isEmpty()) {
    ...
    private <T> T executeWithRetry(java.util.function.Supplier<T> operation, String operationName) {
        ...
        for (int attempt = 1; attempt <= retryAttempts; attempt++) {
            try {
                ...
                lastException = e;
                log.warn("Binom API {} attempt {} failed: {}", operationName, attempt, e.getMessage());
        ...
        throw new RuntimeException("Binom API " + operationName + " failed after " + retryAttempts + " attempts", lastException);
    }
