 ...
 * 
 * SECURITY FEATURES:
 * - Enforces global quantity limits (100 - 1,000,000)
 * - Service-specific limit validation
 * - Fraud detection for suspicious quantities
 * - Bulk order protection
 * - Performance impact assessment
 */
@Slf4j
@Component
public class OrderQuantityRangeValidator implements ConstraintValidator<OrderQuantityRange, Integer> {
    ...
    private boolean validateBasicRange(Integer quantity, ConstraintValidatorContext context) {
        if (quantity < min) {
            addConstraintViolation(context, 
                String.format("Minimum order quantity is %,d", min));
            return false;
        }
        
        if (quantity > max) {
            addConstraintViolation(context, 
                String.format("Maximum order quantity is %,d", max));
            return false;
        }
    ...
    private boolean validateFraudDetection(Integer quantity, ConstraintValidatorContext context) {
        if (quantity > HIGH_RISK_QUANTITY_THRESHOLD) {
            ...
            addConstraintViolation(context, 
                "Quantity exceeds high-risk threshold. Please contact support for large orders.");
            return false;
        }
        ...
        if (isSuspiciousQuantity(quantity)) {
            ...
            addConstraintViolation(context, 
                "Quantity appears suspicious. Please verify your order requirements.");
            return false;
        }
     ...
     */
    private boolean validateServiceSpecificLimits(Integer quantity, ConstraintValidatorContext context) {
        try {
            ...
            log.debug("Service-specific validation would be applied here for quantity: {}", quantity);
     ...
     */
    private boolean validateAgainstServiceLimits(Integer quantity, Long serviceId, 
        ...
        try {
            ...
            if (quantity < service.getMinOrder()) {
                addConstraintViolation(context, 
                    String.format("Service minimum order quantity is %,d", service.getMinOrder()));
                return false;
            }
            
            if (quantity > service.getMaxOrder()) {
                addConstraintViolation(context, 
                    String.format("Service maximum order quantity is %,d", service.getMaxOrder()));
                return false;
            }
            
            return true;
        } catch (Exception e) {
            log.error("Error validating service limits for service {}: {}", serviceId, e.getMessage());
            // Don't fail validation on technical errors
            return true;
        }
    }
