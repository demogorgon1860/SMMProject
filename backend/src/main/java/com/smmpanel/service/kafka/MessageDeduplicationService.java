...
public class MessageDeduplicationService {
    ...
    public boolean isOrderEventAlreadyProcessed(String messageId, Long orderId) {
        ...
        if (existingOrderId != null) {
            ...
            if (orderId.toString().equals(existingOrderId)) {
                ...
                log.warn(
                        "Message ID collision detected: messageId={}, stored orderId={}, current"
                                + " orderId={}",
                        messageId,
                        existingOrderId,
                        orderId);
    ...
    public boolean isVideoProcessingAlreadyProcessed(String messageId, Long videoProcessingId) {
        ...
        if (existingId != null) {
            if (videoProcessingId.toString().equals(existingId)) {
                ...
                log.warn(
                        "Video processing message ID collision: messageId={}, stored ID={}, current"
                                + " ID={}",
                        messageId,
                        existingId,
                        videoProcessingId);
    ...
    public boolean isPaymentConfirmationAlreadyProcessed(String messageId, String transactionId) {
        ...
        if (existingTransactionId != null) {
            if (transactionId.equals(existingTransactionId)) {
                ...
                log.warn(
                        "Payment confirmation message ID collision: messageId={}, stored txId={},"
                                + " current txId={}",
                        messageId,
     ...
     */
    public String generateMessageIdWithTimestamp(
     ...
     */
    public boolean isMessageAlreadyProcessed(
            String messageType, String messageId, String entityId) {
        String prefix =
        ...
        if (existingEntityId != null && entityId.equals(existingEntityId)) {
            log.debug(
                    "Message already processed: type={}, messageId={}, entityId={}",
                    messageType,
                    messageId,
                    entityId);
            return true;
    ...
    public void markMessageAsProcessed(
            String messageType, String messageId, String entityId, Duration ttl) {
        String prefix =
                switch (messageType.toLowerCase()) {
                    case "order" -> ORDER_EVENT_PREFIX;
                    case "video" -> VIDEO_PROCESSING_PREFIX;
        ...
        log.debug(
                "Marked message as processed: type={}, messageId={}, entityId={}, ttl={}",
                messageType,
                messageId,
                entityId,
                ttl);
    ...
    /**
     * Clean up expired deduplication records (manual cleanup if needed) This is typically handled
     * automatically by Redis TTL, but can be called manually
     */
    public void cleanupExpiredRecords() {
        try {
            ...
            log.info(
                    "Deduplication cache status - Order keys: {}, Video keys: {}, Payment keys: {}",
                    orderKeys,
                    videoKeys,
                    paymentKeys);
