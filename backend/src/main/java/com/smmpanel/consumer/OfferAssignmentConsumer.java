...
public class OfferAssignmentConsumer {
    ...
    public void handleOfferAssignmentEvent(
        ...
        try {
            log.info(
                    "Processing offer assignment event for order {}: {}",
                    event.getOrderId(),
                    event.getOfferName());
            ...
            if ("SUCCESS".equals(response.getStatus())) {
                log.info(
                        "Successfully processed offer assignment for order {}: {} campaigns"
                                + " created",
                        event.getOrderId(),
                        response.getCampaignsCreated());
            } else {
                log.error(
                        "Failed to process offer assignment for order {}: {}",
                        event.getOrderId(),
                        response.getMessage());
            }
            ...
            log.error(
                    "Error processing offer assignment event for order {}: {}",
                    event.getOrderId(),
                    e.getMessage(),
                    e);
