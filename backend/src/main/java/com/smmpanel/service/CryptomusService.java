...
import com.smmpanel.repository.BalanceDepositRepository;
...
public class CryptomusService {
    ...
    public CreateDepositResponse createPayment(String username, CreateDepositRequest request) {
        ...
        try {
            CreatePaymentResponse cryptomusResponse = cryptomusClient.createPayment(cryptomusRequest);
            ...
            log.info("Created deposit {} for user {} amount ${}", orderId, username, request.getAmount());
            ...
            log.error("Failed to create Cryptomus payment for user {}: {}", username, e.getMessage(), e);
    ...
    public DepositStatusResponse getPaymentStatus(String username, String orderId) {
        ...
        BalanceDeposit deposit = depositRepository.findByOrderIdAndUser(orderId, user)
    ...
    public Map<String, Object> getUserDeposits(String username, int page, int size) {
        ...
        Page<BalanceDeposit> deposits = depositRepository.findByUserOrderByCreatedAtDesc(
    ...
    public void processWebhook(CryptomusWebhook webhook) {
        try {
            ...
            BalanceDeposit deposit = depositRepository.findByOrderId(webhook.getOrderId())
                    .orElseThrow(() -> new IllegalArgumentException("Deposit not found: " + webhook.getOrderId()));
            ...
            switch (webhook.getStatus()) {
                ...
                case "paid_over":
                    ...
                    log.info("Processed deposit {} for user {} amount ${}", 
                            webhook.getOrderId(), 
                ...
                case "cancel":
                    ...
                    log.info("Deposit {} failed for user {}", 
                            webhook.getOrderId(), 
                            deposit.getUser().getUsername());
                    break;
                ...
                default:
                    log.warn("Unknown webhook status: {} for deposit {}", webhook.getStatus(), webhook.getOrderId());
            }
            ...
            log.error("Failed to process webhook for order {}: {}", webhook.getOrderId(), e.getMessage(), e);
            throw e;
        }
    }
}