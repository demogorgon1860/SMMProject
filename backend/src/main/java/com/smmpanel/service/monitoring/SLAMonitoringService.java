...
import com.smmpanel.service.monitoring.AlertService;
...
public class SLAMonitoringService {
    ...
    public void monitorOrderProcessingSLA() {
        ...
        if (!delayedOrders.isEmpty()) {
            alertService.sendAlert(
                AlertLevel.WARNING,
                String.format("%d orders exceeding processing SLA (%d minutes)", 
                    delayedOrders.size(), 
                    ORDER_PROCESSING_SLA.toMinutes()),
                Map.of("order_ids", delayedOrders.stream()
                    .map(Order::getId)
    ...
    public void monitorOrderCompletionSLA() {
        ...
        List<Order> delayedActiveOrders = orderRepository.findByStatusInAndCreatedAtBefore(
            List.of(OrderStatus.ACTIVE, OrderStatus.PROCESSING, OrderStatus.IN_PROGRESS),
            threshold
        );
        ...
        if (!delayedActiveOrders.isEmpty()) {
            alertService.sendAlert(
                AlertLevel.CRITICAL,
                String.format("%d orders exceeding completion SLA (%d hours)", 
                    delayedActiveOrders.size(), 
                    ORDER_COMPLETION_SLA.toHours()),
                Map.of("order_ids", delayedActiveOrders.stream()
                    .map(Order::getId)
                    .collect(Collectors.toList()))
            );
            ...
            delayedActiveOrders.stream()
                .filter(order -> order.getCreatedAt().isBefore(LocalDateTime.now().minusHours(48)))
    ...
    public void monitorSuccessRate() {
        ...
        long successfulOrders = orderRepository.countByStatusAndCreatedAtAfter(
        ...
        if (totalOrders > 0) {
            ...
            if (successRate < SUCCESS_RATE_THRESHOLD) {
                alertService.sendAlert(
                    AlertLevel.CRITICAL,
                    String.format("Order success rate below threshold: %.2f%% (threshold: %.2f%%)", 
                        successRate * 100, 
    ...
    public void handleOrderStatusChange(OrderStatusChangedEvent event) {
        ...
        if (event.getNewStatus() == OrderStatus.COMPLETED) {
            Duration completionTime = Duration.between(
                order.getCreatedAt(), 
                LocalDateTime.now()
            );
    ...
    private void checkForSuspiciousPatterns(Order order) {
        ...
        long userOrderCount = orderRepository.countByUserIdAndCreatedAtAfter(
            order.getUser().getId(), 
            lastHour
        );
        ...
        if (userOrderCount > SUSPICIOUS_ORDERS_THRESHOLD) {
            alertService.sendAlert(
                AlertLevel.WARNING,
                String.format("User %d has placed %d orders in the last hour", 
                    order.getUser().getId(), 
                    userOrderCount),
                Map.of(
    ...
    private void escalateOrder(Order order) {
        try {
            ...
            alertService.sendAlert(
                AlertLevel.CRITICAL,
                String.format("Order %d escalated due to extended delay (%.1f hours)", 
                    order.getId(),
