...
/**
 * Controller for handling currency-related operations including conversion, formatting, and user
...
@Tag(
        name = "Currency",
        description = "Endpoints for currency conversion, formatting, and preference management")
...
public class CurrencyController {
    ...
    @Operation(
            ...
            description =
                    "Retrieves a list of all supported currencies with their symbols and decimal"
                            + " places")
    public ResponseEntity<ApiResponse<Map<String, CurrencyService.CurrencyInfo>>>
    ...
    @Operation(
            summary = "Get supported currencies with user preference",
            description =
                    "Retrieves a list of all supported currencies with the user's preferred"
                            + " currency marked")
    public ResponseEntity<ApiResponse<List<CurrencyService.CurrencyInfo>>>
            getSupportedCurrenciesWithPreference(@CurrentUser User user) {

        return ResponseEntity.ok(
                ApiResponse.success(
                        userCurrencyPreferenceService.getSupportedCurrenciesWithUserPreference(
                                user.getId())));
    }
    ...
    public ResponseEntity<ApiResponse<CurrencyConversionResultDto>> convertCurrency(
            @CurrentUser User user, @Valid @RequestBody CurrencyConversionRequest request) {
    ...
    public ResponseEntity<ApiResponse<String>> getPreferredCurrency(@CurrentUser User user) {
    ...
    public ResponseEntity<ApiResponse<Void>> updatePreferredCurrency(
            @CurrentUser User user, @RequestParam @NotBlank String currencyCode) {

        userCurrencyPreferenceService.updateUserPreferredCurrency(user.getId(), currencyCode);
        return ResponseEntity.ok(
                ApiResponse.success(null, "Preferred currency updated to " + currencyCode));
    }
    ...
    @PreAuthorize("isAuthenticated()")
    @Operation(
            summary = "Convert to user's currency",
