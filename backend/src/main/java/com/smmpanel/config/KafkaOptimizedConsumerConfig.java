 ...
 * - Order events: 1000 messages/second
 * - Video processing: 100 messages/second with batching
 * - Payment confirmations: Real-time processing
 * 
 * Features:
 * - Snappy compression enabled
 * - Optimized batch sizes and linger settings
 * - Proper acknowledgment modes
...
public class KafkaOptimizedConsumerConfig {
    ...
    public ConsumerFactory<String, Object> orderEventConsumerFactory() {
        ...
        props.put(ConsumerConfig.FETCH_MIN_BYTES_CONFIG, 50000); // Wait for more data before returning
        props.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 100); // Low latency for real-time processing
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 45000); // Longer timeout for stability
        props.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG, 3000);
        props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 600000); // 10 minutes for batch processing
        
        // Performance optimizations
        props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false); // Manual commit for reliability
        props.put(ConsumerConfig.ISOLATION_LEVEL_CONFIG, "read_committed"); // Exactly-once semantics
    ...
    public ConsumerFactory<String, Object> videoProcessingBatchConsumerFactory() {
        ...
        props.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 500); // Balance between throughput and latency
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 30000);
        props.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG, 3000);
        props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 900000); // 15 minutes for video processing
    ...
    public ConsumerFactory<String, Object> paymentConfirmationConsumerFactory() {
        ...
        props.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, 1); // Process one at a time for minimum latency
        props.put(ConsumerConfig.FETCH_MIN_BYTES_CONFIG, 1); // Return immediately when data available
        props.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 10); // Minimal wait time
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 10000); // Shorter timeout for quick failover
    ...
    public ConcurrentKafkaListenerContainerFactory<String, Object> orderEventContainerFactory() {
        ...
        containerProps.setAckMode(ContainerProperties.AckMode.MANUAL_IMMEDIATE); // Fast acknowledgment
        ...
        log.info("Configured order event container factory with 8 concurrent consumers for 1000 msg/sec");
        return factory;
    }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, Object> videoProcessingBatchContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, Object> factory = 
            new ConcurrentKafkaListenerContainerFactory<>();
        ...
        log.info("Configured video processing batch container factory with 4 concurrent consumers and batch processing");
        return factory;
    }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, Object> paymentConfirmationContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, Object> factory = 
            new ConcurrentKafkaListenerContainerFactory<>();
        ...
        log.info("Configured payment confirmation container factory with 2 concurrent consumers for real-time processing");
        return factory;
    }
}