...
/**
 * Integration tests for optimistic locking with real database operations Tests the complete flow
 * from database migration to service layer
 */
@SpringBootTest
@ActiveProfiles("test")
@Slf4j
public class OptimisticLockingIntegrationTest {
    ...
    public void setupIntegrationTest() {
        ...
        log.info(
                "Integration test setup completed - User ID: {}, Service ID: {}",
                testUser.getId(),
                testService.getId());
    }
    ...
    public void testUserServiceBalanceUpdate() {
        ...
        User updatedUser =
                userService.updateBalanceWithLocking(
                        testUser.getId(), depositAmount, "Test deposit");
    ...
    public void testConcurrentBalanceUpdates() throws ExecutionException, InterruptedException {
        ...
        log.info(
                "Final user balance: {}, version: {}",
                finalUser.getBalance(),
                finalUser.getVersion());

        // Balance should have increased by at least one of the amounts
        assertTrue(
                finalUser.getBalance().compareTo(BigDecimal.valueOf(500)) > 0,
    ...
    public void testDeductBalanceForOrder() {
        ...
        BigDecimal initialBalance =
                userRepository.findById(testUser.getId()).map(User::getBalance).orElseThrow();

        log.info("Initial balance: {}", initialBalance);

        // Create order and deduct balance
        BigDecimal orderAmount = BigDecimal.valueOf(50);
        User updatedUser = userService.deductBalanceForOrder(testUser.getId(), orderAmount, 999L);
    ...
    public void testVersionConsistencyAfterMultipleOperations() {
        ...
        for (int i = 1; i <= 5; i++) {
            User updatedUser =
                    userService.updateBalanceWithLocking(
                            userId, BigDecimal.valueOf(10), "Operation " + i);

            Long currentVersion = updatedUser.getVersion();
            log.info(
                    "Operation {}: Version = {}, Balance = {}",
                    i,
                    currentVersion,
                    updatedUser.getBalance());
