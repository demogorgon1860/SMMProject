 ...
 * 
 * Validates that our async processing setup can handle realistic workloads:
 * 1. Thread pools are correctly configured and operational
 * 2. Concurrent task execution works as expected  
 * 3. Performance metrics meet requirements under load
 * 4. Error handling and recovery works properly
 */
...
class AsyncProcessingIntegrationTest {
    ...
    void testAsyncThreadPoolsUnderLoad() throws InterruptedException {
        ...
        for (int i = 0; i < taskCount; i++) {
            ...
            CompletableFuture.runAsync(() -> {
                ...
                try {
                    ...
                    assertNotEquals(mainThreadName, workerThread, 
                            "Task should execute in different thread");
                    assertTrue(workerThread.contains("VideoProcessing"), 
        ...
        assertFalse(executor.getThreadPoolExecutor().isShutdown(), 
                "Thread pool should remain healthy");
        assertTrue(executor.getThreadPoolExecutor().getCompletedTaskCount() >= taskCount,
                "All tasks should be completed");

        System.out.printf("ASYNC INTEGRATION TEST RESULTS:%n" +
                ...
                totalTimeSeconds, throughput, avgTaskTime,
    ...
    void testLightweightExecutorPerformance() throws InterruptedException {
        ...
        for (int i = 0; i < monitoringTaskCount; i++) {
            CompletableFuture.runAsync(() -> {
                try {
                    ...
                    String workerThread = Thread.currentThread().getName();
                    assertTrue(workerThread.contains("Lightweight"), 
        ...
        assertEquals(monitoringTaskCount, successCount.get(), "All monitoring tasks should succeed");

        double totalTimeSeconds = (endTime - startTime) / 1000.0;
        double throughput = successCount.get() / totalTimeSeconds;

        // Monitoring tasks should be very fast
        assertTrue(throughput >= 15.0, 
                "Lightweight task throughput should be high. Actual: " + throughput);
        assertTrue(totalTimeSeconds <= 5.0, 
                "Monitoring tasks should complete within 5 seconds");

        System.out.printf("LIGHTWEIGHT EXECUTOR TEST RESULTS:%n" +
    ...
    void testMixedWorkloadHandling() throws InterruptedException {
        ...
        for (int i = 0; i < heavyTasks; i++) {
            CompletableFuture.runAsync(() -> {
                try {
                    assertTrue(Thread.currentThread().getName().contains("VideoProcessing"));
        ...
        double totalTimeSeconds = (endTime - startTime) / 1000.0;
        assertTrue(totalTimeSeconds <= 15.0, 
        ...
        System.out.printf("MIXED WORKLOAD TEST RESULTS:%n" +
                ...
                heavySuccessCount.get(), heavyTasks,
                lightSuccessCount.get(), lightTasks,
                totalTimeSeconds);
    }
    ...
    void testErrorHandlingAndRecovery() throws InterruptedException {
        ...
        assertEquals(totalTasks - expectedFailures, successCount.get(), 
        ...
        assertTrue(finalCompletedCount >= initialCompletedCount + totalTasks,
